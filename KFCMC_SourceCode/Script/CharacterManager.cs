...
public class CharacterManager : SingletonMonoBehaviour<CharacterManager>
{
    ...
    public class FadeTextSet
    {
        ...
        public FadeTextSet(TMP_Text text, RectTransform rectTrans, CanvasRenderer canvasRenderer, float lifeTime, float alphaRate = 1f)
        {
            this.text = text;
    ...
    public TMP_Text goldNumText;
    public Image goldIcon;
    public Image memoryFragmentIcon;
    public UIRingEffect memoryFragmentRingEffect;
    ...
    public UIRingEffect helpRingEffect;
    ...
    public TMP_Text skillPercentageText;
    public TMP_ColorGradient skillPercentageColorCharged;
    public TMP_ColorGradient skillPercentageColorUncharged;

    // Combo Rank
    public Canvas comboRankCanvas;
    public Image[] comboRankImages;
    public Image comboHealGauge;
    public UIRingEffect comboHealRingEffect;

    private float comboRankDuration;
    private Color comboRankColor = new Color(1, 1, 1, 0);
    private float comboPoint;
    private int comboRank = -1;
    private float comboContinueTimeRemain;
    private float comboHealPoint;
    private const float comboHealMax = 400;
    private static readonly float[] comboRankBorder = { 40, 80, 160, 320 };
    private static readonly float[] comboHealMultiplier = { 1.25f, 1.5f, 1.75f, 2f };
    private static readonly float[] comboRankExpBonus = { 0.25f, 0.5f, 1f, 2f };
    private static readonly string[] comboRankName = { "D", "C", "B", "A" };

    // Reaper
    public Canvas reaperCanvas;
    public TMP_Text reaperText;
    public TMP_ColorGradient[] reaperTextColorGradients;
    ...
    [System.NonSerialized]
    public SearchArea playerSearchArea;
    [System.NonSerialized]
    ...
    public PlayerController pCon
    {
        get
        {
            if (!pConInternal)
            {
                ...
                if (!playerObj)
                {
                    bool isPlayerHyper = false;
                    if (GameManager.Instance)
                    {
                        playerIndex = GameManager.Instance.save.playerIndex;
                        isPlayerHyper = GameManager.Instance.save.isPlayerHyper != 0;
                    }
                    playerObj = Instantiate(CharacterDatabase.Instance.GetPlayer(playerIndex));
                    pConInternal = playerObj.GetComponent<PlayerController>();
                    pConInternal.SetHyper(isPlayerHyper);
                }
                ...
                playerSearchArea = pConInternal.searchArea[0];
    ...
    int[] friendsOrder = new int[GameManager.friendsMax];
    int[] friendsGridSize = new int[GameManager.friendsMax + 1];
    float[] friendsGridScale = new float[GameManager.friendsMax + 1];
    ...
    public const int maxHPBase = 100;
    public const float maxHPLevelRate = 0.05f;
    ...
    bool isHellModeForPlayer = false;
    bool isHellModeForFriends = false;
    int skillPercentageSave = -100;
    ...
    // ForOtherFriends
    public const int friendsID_Serval = 32;
    public const int friendsID_HyperServal = 33;
    ...
    protected override void Awake()
    {
        ...
        InitLoudSound();
        InitRecords();
        friendsAppearPosLayerMask = LayerMask.GetMask("Default", "Field", "EnemyCollision", "SecondField", "ThirdField");
    ...
    public void SetNewPlayer(int newPlayerIndex = 0, bool writeSave = true, bool setHyper = false)
    {
        ...
        if (pConInternal)
        {
            isSuperman = pConInternal.isSuperman;
        }
        if (playerTrans)
        {
        ...
        playerObj = Instantiate(CharacterDatabase.Instance.GetPlayer(playerIndex), defaultPosition, defaultRotation);
        pConInternal = playerObj.GetComponent<PlayerController>();
        playerTrans = playerObj.transform;
        playerLookAt = pConInternal.lookAtTarget;
        playerSearchTarget = pConInternal.searchTarget[0].transform;
        ...
        if (setHyper)
        {
            pConInternal.SetHyper();
        }
        if (isSuperman)
        {
            pConInternal.SupermanStart(false);
        }
        ...
        if (pIconsMother)
        {
            if (playerIndex == GameManager.playerIndex_HyperServal || setHyper)
            {
        ...
        if (writeSave)
        {
            GameManager.Instance.save.playerIndex = newPlayerIndex;
            GameManager.Instance.save.isPlayerHyper = (setHyper ? 1 : 0);
        }
    }

    public void RestorePlayer()
    {
        SetNewPlayer(GameManager.Instance.save.playerIndex, false);
    }
    ...
    void Start()
    {
        ...
        if (pIconsMother)
        {
            if (playerIndex == GameManager.playerIndex_HyperServal)
            {
                pIconsMother.SetSpriteType(1);
            }
            else
    ...
    void InitLoudSound()
    {
        ...
        loudSoundVolumeIncrease[(int)LoudSoundType.Explosion1] = 0.75f / 0.07f;
    }
    ...
    void Update()
    {
        ...
        if (!playerDead)
        {
            ...
            if (PauseController.Instance.pauseEnabled && !PauseController.Instance.pauseGame && Time.timeScale > 0f && playerIndex != GameManager.playerIndex_Parkman && pCon && pCon.controlEnabled)
            {
                ChangeCommand();
            }
            UpdateStatusSlot();
        ...
        // Combo Rank
        if (comboContinueTimeRemain > 0)
        {
            comboContinueTimeRemain -= deltaTimeCache;
            if (comboContinueTimeRemain <= 0)
            {
                ResetComboPoint();
            }
        }
        comboRankDuration += deltaTimeCache;
        if (comboRank >= 0)
        {
            if (comboRankColor.a < 1f)
            {
                comboRankColor.a = Mathf.Clamp01(comboRankDuration * 8f);
                comboRankImages[comboRank].color = comboRankColor;
            }
        }
        else
        {
            if (comboRankColor.a > 0f)
            {
                comboRankColor.a = Mathf.Clamp01(1f - comboRankDuration * 2f);
                for (int i = 0; i < comboRankImages.Length; i++)
                {
                    if (comboRankImages[i].enabled)
                    {
                        comboRankImages[i].color = comboRankColor;
                        if (comboRankColor.a <= 0f)
                        {
                            comboRankImages[i].enabled = false;
                        }
                    }
                }
            }
    ...
    private void LateUpdate()
    {
        ...
        {
            int spriteIndex = (StageManager.Instance.IsWithoutBringItemStage ? 1 : 0);
            ...
            if (helpMarkImage.enabled != showHelpMark)
            {
                ...
                if (showHelpMark)
                {
                    helpRingEffect.StartEffect();
                }
                else
                {
                    helpRingEffect.EndEffect();
                }
            }
        }
        if (assistExpertMarks.Length >= 4)
        {
            ...
            if (GameManager.Instance.save.config[GameManager.Save.configID_HellMode] != (int)ConfigConsts.HellMode.Off)
            {
            ...
            if (GameManager.Instance.save.config[GameManager.Save.configID_DisableInvincibility] != (int)ConfigConsts.DisableInvincibility.Off)
            {
    ...
    public void UpdateGoldPos()
    {
        if (goldCanvas)
        {
            ...
            {
                RectTransform goldRect = goldCanvas.GetComponent<RectTransform>();
                ...
                if (!goldCanvas.enabled && (goldTimeRemain > 0f || showGoldContinuous))
                {
                    goldCanvas.enabled = true;
                }
            }
            else
            {
    ...
    public void RestoreFriends()
    {
        ...
        for (int i = 0; i < friends.Length; i++)
        {
            if (GameManager.Instance.save.friendsLiving[i] != 0)
            {
                ...
                if (friends[i].fBase)
                {
                    ...
                    if (GameManager.Instance.save.servalExLevel > 0 && IsServalID(i))
                    {
                        Friends_Serval servalComponent = friends[i].fBase.GetComponent<Friends_Serval>();
                        if (servalComponent)
                        {
                            servalComponent.SetServalLevel(GameManager.Instance.save.servalExLevel);
                            isServalExist = true;
                        }
                    }
                }
        ...
        ResetHP();
        if (float.IsInfinity(GameManager.Instance.save.sandstar) || float.IsNaN(GameManager.Instance.save.sandstar))
        {
        ...
        if (!isServalExist)
        {
            GameManager.Instance.save.servalExLevel = 0;
        }
    }
        ...
        if (MessageUI.Instance)
        {
            if (index < 0)
            {
                if (pCon)
                {
                    ...
                    if (mesBackColor)
                    {
                        int faceId = CharacterDatabase.Instance.GetPlayerFaceId(playerIndex);
                        MessageUI.Instance.SetMessageOptional(TextManager.Get(talkName), mesBackColor.color1, mesBackColor.color2, mesBackColor.twoToneType, faceId, time, 1, 1, true);
                        MessageUI.Instance.SetMessageLog(TextManager.Get(talkName), faceId);
                    }
                }
            }
            else
    ...
    public void CheckJaparimanShortage(int itemID)
    {
        switch (itemID)
        {
            case (int)ItemID.Japariman:
                japarimanShortage -= 1;
                break;
            case (int)ItemID.Japariman_2:
                japarimanShortage -= 2;
                break;
            case (int)ItemID.Japariman_3:
                japarimanShortage -= 3;
                break;
            case (int)ItemID.Japariman_4:
                japarimanShortage -= 4;
                break;
            case (int)ItemID.Japariman_5:
                japarimanShortage -= 5;
                break;
        }
    ...
    public bool GetFriendsEffectable(int index, bool ignoreConfig = false)
    {
        if (friendsEffectDisabled == 0 || ignoreConfig)
        {
            if (GetFriendsIdOfPlayer == index)
            {
                return true;
            }
            else if (GetFriendsExist(index, true))
            {
    ...
    {
        if (GetFriendsIdOfPlayer == safetyBombID)
        {
            return true;
        }
        else if (GetFriendsExist(safetyBombID, true))
        {
    ...
    public float GetFriendsEffect(FriendsEffect type)
    {
        ...
        switch (type)
        {
            ...
            case FriendsEffect.PPP:
                if (GetFriendsEffectable(16, true)
                    && GetFriendsEffectable(17, true)
                    && GetFriendsEffectable(18, true)
                    && GetFriendsEffectable(19, true)
                    && GetFriendsEffectable(20, true))
                {
                    answer = 1;
                }
                else
    ...
    public float GetDifficultyEffect(DifficultyEffect type, bool forceDefaultDifficulty = false)
    {
        ...
        switch (type)
        {
            ...
            case DifficultyEffect.Reaper:
                if (StageManager.Instance && StageManager.Instance.IsWithoutBringItemStage)
                {
                    answer = 1;
                }
                else if (difficulty <= 2)
    ...
    public void StaminaHeal(float percent, bool allFlag = true, bool attenuate = true)
    {
        if (pCon)
        {
            pCon.AddNowST(percent);
        }
        if (allFlag)
        {
            if (attenuate)
            {
                percent *= riskyDecSqrt;
            }
            for (int i = 0; i < friends.Length; i++)
            {
                if (friends[i].fBase)
                {
                    friends[i].fBase.AddNowST(percent);
                }
            }
        }
    }
    ...
    public void ResetStatus()
    {
        ...
        for (int i = 0; i < friends.Length; i++)
        {
            if (friends[i].fBase)
            {
                friends[i].fBase.ResetStatus();
            }
        }
    }

    public void HealGuts(float rate, int effectNum = -1, bool toCenterPivot = false)
    {
        if (pCon)
        {
            pCon.HealGuts(rate);
    ...
    public void ResetForMoveStage()
    {
        ...
        comboHealPoint = 0;
        UpdateComboHealGauge();
    }

    public void ResetForMoveFloor(bool healDisabled = false)
    {
        if (!healDisabled)
        {
        ...
        if (numberUiInitialized)
        {
            ...
            {
                int count = damageList[canvasIndex].Count;
                if (count > damageTextMax)
                {
                    for (int i = damageTextMax; i < count; i++)
                    {
                        if (damageList[canvasIndex][i].text)
                        {
                            Destroy(damageList[canvasIndex][i].text.gameObject);
                    ...
                    damageList[canvasIndex].RemoveRange(damageTextMax, count - damageTextMax);
                ...
                if (count > 0)
                {
                    for (int i = 0; i < count; i++)
                    {
                        ...
                        {
                            damageList[canvasIndex][i].lifeTime = 0f;
                            damageList[canvasIndex][i].canvasRenderer.SetAlpha(0f);
                        }
                    }
                }
            }
            for (int dodgeType = 0; dodgeType < dodgeTextTypeMax; dodgeType++)
            {
                ...
                if (count > damageTextMax)
                {
                    ...
                    dodgeList[dodgeType].RemoveRange(damageTextMax, count - damageTextMax);
                ...
                if (count > 0)
                {
                    for (int i = 0; i < count; i++)
                    {
                        if (dodgeList[dodgeType][i].lifeTime > 0f)
                        {
                            dodgeList[dodgeType][i].canvasRenderer.SetAlpha(0f);
                        }
                    }
                }
            }
        ...
        {
            comboRankCanvas.enabled = StageManager.Instance.dungeonMother.comboRankEnabled;
        }
    ...
    public void JustDodgeAmountPlus(float amount)
    {
        if (amount > 0f && pCon)
        {
            ...
            if (playerKnock > 0f)
            {
                justDodgeAmount += amount * pCon.GetJustDodgeAmountMultiplier / playerKnock * 0.125f;
    ...
    public void JustDodgeAmountPlusNonAttack(float amount = 1f)
    {
        justDodgeAmount = Mathf.Clamp01(justDodgeAmount + amount);
    }
    ...
    public int GetStandardMaxHPNoEffected()
    {
        return maxHPBase + Mathf.RoundToInt(maxHPBase * maxHPLevelRate * (pCon ? pCon.LevelForStatus : 0)) + GetMaxHPPlus() * maxHPBase / 10;
    }

    public int GetStandardMaxHP()
    {
        return Mathf.RoundToInt(GetStandardMaxHPNoEffected() * (1f + GetFriendsEffect(FriendsEffect.MaxHP)));
    }
    ...
    {
        isHellModeForPlayer = GameManager.Instance.save.config[GameManager.Save.configID_HellMode] != (int)ConfigConsts.HellMode.Off;
        isHellModeForFriends = GameManager.Instance.save.config[GameManager.Save.configID_HellMode] == (int)ConfigConsts.HellMode.On;
        if (isHellModeForPlayer)
        {
    ...
    public bool IsHellMode(bool isPlayer)
    {
        if (isPlayer)
        {
            return isHellModeForPlayer;
        }
        else
        {
            return isHellModeForFriends;
        }
    }
    ...
    {
        int jmCount;
        jmCount = GameManager.Instance.save.NumOfSpecificItems((int)ItemID.Japariman);
        for (int i = 0; i < jmCount && num > 0; i++)
        {
            GameManager.Instance.save.RemoveItem((int)ItemID.Japariman);
            num -= 1;
        }
        if (num > 0)
        {
            jmCount = GameManager.Instance.save.NumOfSpecificItems((int)ItemID.Japariman_2);
            for (int i = 0; i < jmCount && num > 0; i++)
            {
                GameManager.Instance.save.RemoveItem((int)ItemID.Japariman_2);
                num -= 2;
            }
        }
        if (num > 0)
        {
            jmCount = GameManager.Instance.save.NumOfSpecificItems((int)ItemID.Japariman_3);
            for (int i = 0; i < jmCount && num > 0; i++)
            {
                GameManager.Instance.save.RemoveItem((int)ItemID.Japariman_3);
                num -= 3;
            }
        }
        if (num > 0)
        {
            jmCount = GameManager.Instance.save.NumOfSpecificItems((int)ItemID.Japariman_4);
            for (int i = 0; i < jmCount && num > 0; i++)
            {
                GameManager.Instance.save.RemoveItem((int)ItemID.Japariman_4);
                num -= 4;
            }
        }
        if (num > 0)
        {
            jmCount = GameManager.Instance.save.NumOfSpecificItems((int)ItemID.Japariman_5);
            for (int i = 0; i < jmCount && num > 0; i++)
            {
                GameManager.Instance.save.RemoveItem((int)ItemID.Japariman_5);
                num -= 5;
            }
        }
        if (num < 0)
        {
            num *= -1;
            if (num >= 4 && GameManager.Instance.save.IsPossibleAddItem())
            {
                GameManager.Instance.save.AddItem((int)ItemID.Japariman_4);
                num -= 4;
            }
            if (num >= 3 && GameManager.Instance.save.IsPossibleAddItem())
            {
                GameManager.Instance.save.AddItem((int)ItemID.Japariman_3);
                num -= 3;
            }
            if (num >= 2 && GameManager.Instance.save.IsPossibleAddItem())
            {
                GameManager.Instance.save.AddItem((int)ItemID.Japariman_2);
                num -= 2;
            }
            while (num >= 1 && GameManager.Instance.save.IsPossibleAddItem())
            {
                GameManager.Instance.save.AddItem((int)ItemID.Japariman);
                num -= 1;
            }
            while (num >= 1)
            {
                ItemDatabase.Instance.GiveItem((int)ItemID.Japariman, playerTrans.position, 1);
                num -= 1;
            }
        }
    }
    ...
    public Vector3 GetFriendsAppearPos(int numerator = 0, int denominator = -1)
    {
        if (playerTrans)
        {
            ...
            if (Physics.Raycast(ray, out raycastHit, 1f, friendsAppearPosLayerMask, QueryTriggerInteraction.Ignore))
            {
    ...
    public int ChangeFriends(int index, bool effectFlag = true, bool costFlag = true, bool ignoreLimit = false, int numerator = 0, int denominator = -1, bool ignoreSave = false)
    {
        int answer = -3;
        //cost limit -1
        ...
        {
            if ((GameManager.Instance.save.friends[index] != 0 || ignoreSave) && !friends[index].instance)
            {
                int costNum = CharacterDatabase.Instance.friends[index].cost;
                if (costFlag && GameManager.Instance.GetJaparimanCount() < costNum)
                {
                    answer = -2;
                }
                ...
                else
                {
                    ...
                    {
                        friends[index].prefab = CharacterDatabase.Instance.GetFriends(index);
                    }
    ...
    public void ReserveAllFriendsAgentReset()
    {
        StartCoroutine(AllFriendsAgentReset());
    }

    IEnumerator AllFriendsAgentReset()
    {
    ...
    public void WriteFriendsLiving()
    {
        ...
        if (float.IsInfinity(sandstar) || float.IsNaN(sandstar))
        {
            sandstar = 0f;
        }
    ...
    public void ShowGold(bool isMemoryFragment = false)
    {
        if (goldCanvas)
        {
            if (goldIcon.enabled != !isMemoryFragment)
            {
                goldIcon.enabled = !isMemoryFragment;
            }
            if (memoryFragmentIcon.enabled != isMemoryFragment)
            {
                memoryFragmentIcon.enabled = isMemoryFragment;
            }
            if (isMemoryFragment)
            {
                goldNumText.text = GameManager.Instance.save.memoryFragment.ToString();
            }
            else
            {
                goldNumText.text = GameManager.Instance.save.money.ToString();
            }
    ...
    public int GetIbisSongHeal()
    {
        if (pCon)
        {
            return Mathf.RoundToInt(GetStandardMaxHP() * GetDifficultyEffect(DifficultyEffect.IbisHealRate) * riskyDecCubicRoot);
        }
        else
        {
            return 0;
        ...
        if (damageSize > 0 && (mainCamera || GetMainCamera()))
        {
            ...
            if (listCount > 0)
            {
                for (int i = 0; i < listCount; i++)
                {
                    ...
                    else if (damageList[canvasIndex][i].lifeTime <= 0f)
                    {
                        ...
                        damageList[canvasIndex][i].canvasRenderer.SetAlpha(1f);
            ...
            if (!found)
            {
                if (listCount < 128)
                {
                    ...
                    CanvasRenderer canvasRenderer = damageInstance.GetComponent<CanvasRenderer>();
                    ...
                    damageList[canvasIndex].Add(new FadeTextSet(text, rectTrans, canvasRenderer, damageTextLife, 1f));
                }
                else
                {
                    int minIndex = -1;
                    ...
                    if (minIndex >= 0)
                    {
                        ...
                        damageList[canvasIndex][minIndex].canvasRenderer.SetAlpha(1f);
                    ...
                    for (int i = 0; i < listCount; i++)
                    {
                        ...
                        else if (damageList[canvasIndex][i].lifeTime > 0f)
                        {
                            ...
                            else if (damageList[canvasIndex][i].lifeTime < 1f)
                            {
                                ...
                                damageList[canvasIndex][i].canvasRenderer.SetAlpha(damageList[canvasIndex][i].lifeTime);
                            }
                        }
                    }
                }
    ...
    void SetDodgeText(int type, Vector3 pos, bool changeText, string textContent = "", float lifeTimeRate = 1f, float alphaRate = 1f)
    {
        ...
        if (listCount > 0)
        {
            for (int i = 0; i < listCount; i++)
            {
                ...
                else if (dodgeList[type][i].lifeTime <= 0f)
                {
                    dodgeList[type][i].rectTrans.position = pos;
                    dodgeList[type][i].canvasRenderer.SetAlpha(1f);
                    dodgeList[type][i].lifeTime = dodgeTextLife * lifeTimeRate;
                    dodgeList[type][i].alphaRate = alphaRate;
                    if (changeText)
                    {
                        dodgeList[type][i].text.text = textContent;
                    }
                    found = true;
                    break;
                }
        ...
        if (!found)
        {
            if (listCount < 128)
            {
                ...
                CanvasRenderer canvasRenderer = dodgeInstance.GetComponent<CanvasRenderer>();
                rectTrans.position = pos;
                ...
                dodgeList[type].Add(new FadeTextSet(text, rectTrans, canvasRenderer, dodgeTextLife * lifeTimeRate, alphaRate));
            }
            else
            {
                int minIndex = -1;
                ...
                if (listCount > 0)
                {
                    for (int i = 0; i < listCount; i++)
                    {
                        ...
                        else if (dodgeList[type][i].lifeTime > 0f)
                        {
                            ...
                            {
                                dodgeList[type][i].canvasRenderer.SetAlpha(0f);
                            }
                            else
                            {
                                dodgeList[type][i].rectTrans.Translate(50f * deltaTimeCache * vecUp);
                                dodgeList[type][i].canvasRenderer.SetAlpha(Mathf.Clamp01(dodgeList[type][i].lifeTime * dodgeList[type][i].alphaRate * dodgeTextAlphaSpeed));
                            }
                        }
                    }
                }
    ...
    public void ShowJustDodge(JustDodgeType justDodgeType, AttackDetection attacker = null)
    {
        if (pCon && playerTrans && pCon.GetNowHP() > 0)
        {
            if (mainCamera || GetMainCamera())
            {
                ...
                SetDodgeText(justDodgeAmount >= 1f ? dodgeText_JustDodgeHigh : justDodgeAmount > 0f ? dodgeText_JustDodgeMiddle : dodgeText_JustDodgeLow, pos, false);
            }
    ...
    public void ShowGuts(Vector3 position, bool isLast = false)
    {
        if (mainCamera || GetMainCamera())
        {
            ...
            SetDodgeText(isLast ? dodgeText_GutsLast : dodgeText_GutsNormal, pos, false);
        }
    }

    public void ShowGetExp(Vector3 position, int exp, int comboRank)
    {
        if ((mainCamera || GetMainCamera()) && exp > 0)
        {
            ...
            if (comboRank >= 0)
            {
                text = StringUtils.Format("{0}Exp<b><color=white><gradient=\"ComboRank_{1}_Gradient\">*{1}</gradient></color></b>", exp, comboRankName[comboRank]);
            }
            else
            {
                text = StringUtils.Format("{0}Exp", exp);
            }
            SetDodgeText(dodgeText_Exp, pos, true, text);
        }
    }
    ...
    public void ShowDefeatRemain(Vector3 position)
    {
        if (mainCamera || GetMainCamera())
        {
            ...
            {
                string text = sb.Clear().AppendLine().Append(StageManager.Instance.dungeonController.GetDefeatMissionConditionText()).ToString();
                SetDodgeText(dodgeText_DefeatRemain, pos, true, text);
            }
        }
    }
    ...
    public void ShowFieldBuff(Vector3 position, FriendsBase.FieldBuffType type, bool isPlayer)
    {
        if (isPlayer || GameManager.Instance.save.config[GameManager.Save.configID_ObscureFriends] <= 2)
        {
            ...
            SetDodgeText(dodgeText_FieldBuffHP + (int)type, pos, false, "", sizeFloat, 1f / 0.8f);
        }
    }
    ...
    public void ShowBossResult(int enemyID, bool sinWR)
    {
        if (enemyID >= 0)
        {
            ...
            if (!GameManager.Instance.IsStandardPlayer())
            {
                bossResult.iconImages[iconCount].sprite = ItemDatabase.Instance.GetItemImage(CharacterDatabase.Instance.GetPlayerFaceId(GameManager.Instance.save.playerIndex));
                iconCount++;
            }
    ...
    {
        if (IsServalID(index) && GameManager.Instance.save.servalExLevel > 0)
        {
            return GameManager.Instance.save.servalExLevel;
        }
        else
        {
            if (extraFriendsIndex > 0 && index == extraFriendsID[0])
    ...
    public void SetSkillPercentageEnabled(bool enabled)
    {
        if (skillPercentageText)
        {
            skillPercentageText.enabled = enabled;
            if (!enabled)
            {
                skillPercentageText.text = "";
                skillPercentageSave = -1;
            }
        }
    }

    public void SetSkillPercentage(int percentage)
    {
        if (skillPercentageText)
        {
            if (percentage < 0)
            ...
            if (percentage != skillPercentageSave)
            {
                skillPercentageText.text = percentage + "%";
                if (percentage >= 100)
                {
                    if (skillPercentageText.colorGradientPreset != skillPercentageColorCharged)
                    {
                        skillPercentageText.colorGradientPreset = skillPercentageColorCharged;
                    }
                }
                else
                {
                    if (skillPercentageText.colorGradientPreset != skillPercentageColorUncharged)
                    {
                        skillPercentageText.colorGradientPreset = skillPercentageColorUncharged;
                    }
                }
            }
        }
    ...
    public int GetFriendsIdOfPlayer
    {
        get
        {
            return CharacterDatabase.Instance.players[playerIndex].friendsId;
        }
    }

    public bool IsPlayerServal
    {
        get
        {
            return playerIndex == GameManager.playerIndex_Serval || playerIndex == GameManager.playerIndex_HyperServal;
        }
    }

    public bool IsPlayerAnotherServal
    {
        get
        {
            return playerIndex == GameManager.playerIndex_AnotherServal || playerIndex == GameManager.playerIndex_AnotherServal_Escape;
        }
    }

    public void CallServal(Vector3 position)
    {
        if (GameManager.Instance.GetJaparimanCount() > 0 || StageManager.Instance.IsHomeStage)
        {
            int servalID = GameManager.Instance.save.isPlayerHyper != 0 ? friendsID_HyperServal : friendsID_Serval;
            Friends_Serval servalComponent = null;
            ...
            if (!GetFriendsExist(servalID, false))
            {
                // Not Exist
                if (GameManager.Instance.save.friends.Length > servalID)
                {
                    GameManager.Instance.save.friends[servalID] = 1;
                }
                ChangeFriends(servalID, true, false, true, 0, -1, true);
                if (friends[servalID].fBase && (servalComponent = friends[servalID].fBase.GetComponent<Friends_Serval>()))
                {
                    servalComponent.SetServalLevel(1);
                }
                if (friends[servalID].trans)
                {
                    friends[servalID].trans.position = position;
                }
            }
            else if (GetFriendsExist(servalID, true))
            {
                // Living
                if (friends[servalID].fBase && (servalComponent = friends[servalID].fBase.GetComponent<Friends_Serval>()))
                {
                    servalComponent.IncrementServalLevel();
                }
            }
            else
            {
                ...
                if (friends[servalID].trans)
                {
                    position = friends[servalID].trans.position;
                    rotation = friends[servalID].trans.rotation;
                }
                Erase(servalID, false);
                ChangeFriends(servalID, true, false, true, 0, -1, true);
                if (friends[servalID].fBase && (servalComponent = friends[servalID].fBase.GetComponent<Friends_Serval>()))
                {
                    servalComponent.SetForRevive();
                    servalComponent.SetServalLevel(1);
                }
                if (friends[servalID].trans)
                {
                    friends[servalID].trans.SetPositionAndRotation(position, rotation);
                }
            }
        }
        else
        {
            MessageUI.Instance.SetMessageReplace(TextManager.Get("CAUTION_JAPARIBUN"), "CAUTION_JAPARIBUN", 4, MessageUI.panelType_Information, MessageUI.slotType_Caution);
        }
    }

    public void ExchangeServal(bool toHyper)
    {
        Friends_Serval servalComponent = null;
        int nowServalID = toHyper ? friendsID_Serval : friendsID_HyperServal;
        int newServalID = toHyper ? friendsID_HyperServal : friendsID_Serval;
        if (GetFriendsExist(nowServalID, true))
        {
            if (friends[nowServalID].fBase && (servalComponent = friends[nowServalID].fBase.GetComponent<Friends_Serval>()))
            {
                Vector3 servalPosition = servalComponent.transform.position;
                Quaternion servalRotation = servalComponent.transform.rotation;
                Erase(nowServalID, false);
                ChangeFriends(newServalID, true, false, true, 0, -1, true);
                if (friends[newServalID].fBase && (servalComponent = friends[newServalID].fBase.GetComponent<Friends_Serval>()))
                {
                    servalComponent.SetServalLevel(GameManager.Instance.save.servalExLevel);
                    servalComponent.transform.SetPositionAndRotation(servalPosition, servalRotation);
                }
            }
        }
        else if (GetFriendsExist(nowServalID, false))
        {
            Erase(nowServalID, false);
        }
    }

    public static bool IsServalID(int friendsID) => (friendsID == friendsID_Serval || friendsID == friendsID_HyperServal);

    public GameObject GetFriendsSearchTarget(int friendsID)
    {
        if (friends[friendsID].fBase)
        {
            GameObject[] searchTargets = friends[friendsID].fBase.GetSearchTarget();
    ...
    public Transform GetFriendsBreathPivot(int friendsID)
    {
        if (friends[friendsID].fBase)
        {
            return friends[friendsID].fBase.breathPivot;
        }
        return null;
    }

    public bool IsPlayerWaveAttacking
    {
        get
        {
            if (pCon && (IsPlayerServal || IsPlayerAnotherServal))
            {
                PlayerController_Serval pConServal = pCon.GetComponent<PlayerController_Serval>();
                if (pConServal)
                {
                    return pConServal.IsWaveAttacking;
                }
            }
            return false;
        }
    ...
    // Combo Rank
    public int AddComboPoint(float point)
    {
        int itemID = -1;
        int newComboRank = -1;

        comboPoint += point;
        comboContinueTimeRemain = 3;

        for (int i = comboRankBorder.Length - 1; i >= 0; i--)
        {
            if (comboPoint >= comboRankBorder[i])
            {
                newComboRank = i;
                break;
            }
        }
        if (comboRank != newComboRank)
        {
            comboRank = newComboRank;
            for (int i = 0; i < comboRankImages.Length; i++)
            {
                comboRankImages[i].enabled = (i == comboRank);
            }
            if (comboRank >= 0)
            {
                comboRankColor.a = 0;
        ...
        if (point > 0 && comboRank >= 0)
        {
            comboHealPoint += point * comboHealMultiplier[comboRank];
            if (comboHealPoint >= comboHealMax)
            {
                comboHealPoint -= comboHealMax;
                itemID = (int)ItemDatabase.ItemID.ComboStar;
                comboHealRingEffect.StartEffect();
            }
            UpdateComboHealGauge();
        }
        return itemID;
    ...
    {
        if (comboPoint > 0 || comboRank >= 0 || comboContinueTimeRemain > 0)
        {
            comboPoint = 0;
            comboRank = -1;
            comboContinueTimeRemain = 0;
            comboRankDuration = 0;
            UpdateComboHealGauge();
        }
    }
    ...
    public float GetComboRankExpBonus()
    {
        if (comboRank >= 0)
        {
            return comboRankExpBonus[Mathf.Clamp(comboRank, 0, comboRankExpBonus.Length - 1)];
        }
        else
        {
            return 0;
    ...
    public bool IsComboContinuing
    {
        get { return comboPoint > 0; }
    }

    // Reaper
    public void SetReaperCanvasEnabled(bool value)
    {
        reaperCanvas.enabled = value;
    }

    public void SetReaperText(string text)
    {
        reaperText.text = text;
    }

    public void SetReaperTextColor(int index)
    {
        if (index >= 0 && index <= reaperTextColorGradients.Length)
        {
            reaperText.colorGradientPreset = reaperTextColorGradients[index];
        }
    }

    void UpdateComboHealGauge()
    {
        if (comboHealGauge.enabled != (comboRank >= 0))
        {
            comboHealGauge.enabled = (comboRank >= 0);
        }
        comboHealGauge.fillAmount = Mathf.Clamp01(comboHealPoint / comboHealMax);
    }

    public void CheckMemoryFragmentMax()
    {
        if (GameManager.Instance.save.memoryFragment >= GameManager.memoryFragmentMax)
        {
            EmitEffect((int)EffectDatabase.id.getMemoryOrb, -1, false, true);
            ItemDatabase.Instance.GiveItem((int)ItemDatabase.ItemID.MemoryOrb,
                playerTrans.position + Vector3.up * 0.7f, 5, -1, -1, 0f, StageManager.Instance.dungeonController.itemSettings.container, 0);
            GameManager.Instance.save.memoryFragment -= GameManager.memoryFragmentMax;
            memoryFragmentRingEffect.StartEffect();
        }
    }

    public ItemDatabase.ItemID GetHealStarOrSandstarBlockItemID()
    {
        // アミュレット非装備、またはサンドスターが最大
        if (GetEquipEffect(EquipEffect.WildRelease) == 0 || GetSandstarIsMax())
        {
            // 確定でヒールスター
            return ItemDatabase.ItemID.HealStar;
        }
        // フレンズ全員がHP最大
        else if (!GetFriendsInjured())
        {
            // 確定でサンドスターブロック
            return ItemDatabase.ItemID.SandstarBlock;
        }
        else
        {
            // ランダム
            return (Random.Range(0, 2) == 0 ? ItemDatabase.ItemID.HealStar : ItemDatabase.ItemID.SandstarBlock);
        }
    }
