    ...
    void SetChoices()
    {
        ...
        for (int i = 0; i < GameManager.Instance.save.armsLocked.Length; i++)
        {
            if (GameManager.Instance.save.armsLocked[i] >= 1)
            {
                ...
                int lockedArmsID = GameManager.Instance.GetLockedArmsID(i);
                armsObject = TextManager.Get(string.Format("ITEM_NAME_{0:000}", GameManager.armsIDBase + lockedArmsID));
                // インベントリアップの場合は×2を追加
                if (lockedArmsID == GameManager.equipmentID_InventoryUp)
                { 
                    armsObject += "×2";
                }
        ...
        countTemp++;
        PauseController.Instance.SetChoicesDirect(countTemp, true, TextManager.Get("WORD_UNLOCKCONSOLE"), lockedNames[0], lockedNames[1], lockedNames[2], lockedNames[3], lockedNames[4], lockedNames[5]);
    }
