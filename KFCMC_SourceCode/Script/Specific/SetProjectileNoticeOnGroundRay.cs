...
public class SetProjectileNoticeOnGroundRay : MonoBehaviour {
        ...
        if (elapsedTime >= delay) {
            ...
            if (interval <= 0f) {
                ...
                if (sphereCastRadius <= 0f) {
                    if (Physics.Raycast(rayPivot.position, vecDown, out hit, maxDistance, layerMask, QueryTriggerInteraction.Ignore)) {
                        found = true;
                    }
                } else {
                    if (Physics.SphereCast(rayPivot.position, sphereCastRadius, vecDown, out hit, maxDistance, layerMask, QueryTriggerInteraction.Ignore)) {
                        found = true;
                    }
                }
                if (found) {
