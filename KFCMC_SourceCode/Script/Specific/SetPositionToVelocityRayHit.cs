...
public class SetPositionToVelocityRayHit : MonoBehaviour {
    ...
    public float castingSphereRadius;
    ...
    void Update() {
        if (elapsedTime >= delay) {
            ...
            if (checkRigidbody.velocity.sqrMagnitude >= 0.01f) {
                if (castingSphereRadius > 0f)
                {
                    if (Physics.SphereCast(checkRigidbody.position, castingSphereRadius, checkRigidbody.velocity.normalized, out hit, maxDistance, layerMask, collideTriggerEnabled ? QueryTriggerInteraction.Collide : QueryTriggerInteraction.Ignore))
                    {
