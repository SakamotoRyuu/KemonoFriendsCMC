...
public class GiraffeBeam : MonoBehaviour {
    ...
    void SetBeam(bool flag) {
        if (forEnemy || GameManager.Instance.save.config[GameManager.Save.configID_ShowGiraffeBeam] >= (isPlayer ? 1 : 2)) {
            if (flag) {
    ...
    bool CheckCondition() {
        return (attackDetection && ((CharacterManager.Instance && CharacterManager.Instance.GetBuff(CharacterManager.BuffType.Long)) || (forHyper && attackDetection.parentCBase && attackDetection.parentCBase.isSuperman) || (forEnemy && attackDetection.parentCBase)));
    }
