...
public class Event_ClearCaveOfTribulation : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        switch (progress)
        {
            ...
            case 1:
                ...
                break;
            case 2:
                PauseController.Instance.SetBlackCurtain(Mathf.Clamp01(elapsedTime * 0.4f), true);
                if (audioSource)
                {
                    audioSource.volume = Mathf.Clamp01(1f - elapsedTime * 0.4f);
                }
                if (elapsedTime >= 2.5f)
                {
            ...
            case 3:
                ...
                break;
            case 4:
                GameManager.Instance.SetSecret(GameManager.SecretType.CaveOfTribulationsCleared);
                TrophyManager.Instance.CheckTrophy_Clear();
                GameManager.Instance.SetSteamAchievement(GameManager.SteamAchievementName.CLEARCAVEOFTRIBULATIONS);
                elapsedTime = 0f;
                progress++;
                break;
            case 5:
