    ...
    private void Update() {
        ...
        switch (progress) {
            ...
            case 24:
                ...
                {
                    bgmPreloaded = true;
                    BGM.Instance.Preload(newMusicNumber);
                }
                if (elapsedTime >= 2.5f) {
                    ...
                    {
                        CharacterManager.Instance.SetNewPlayer(GameManager.playerIndex_HyperServal, false, true);
                    }
                    else
                    {
                        CharacterManager.Instance.SetNewPlayer(CharacterManager.Instance.playerIndex, false, true);
                    }
            ...
            case 28:
                ...
                if (elapsedTime > 0.75f) {
                    ...
                    {
                        CharacterManager.Instance.pCon.SetFaceString("Blink", 10f);
                    }
                    NextProgress();
                }
                break;
            case 29:
                ...
                if (elapsedTime > 2f) {
                    ...
                    eventCamPivot[3].LookAt(CharacterManager.Instance.playerTrans.position + Vector3.up * 0.1f);
                    CameraManager.Instance.CancelQuake();
                    CameraManager.Instance.SetEventCameraFollowTarget(eventCamPivot[3], 8f, 1.5f, 0.45f);
                    playerEyeHeight = CharacterManager.Instance.pCon.GetEyeHeight + 0.03f;
                    NextProgress();
                }
                break;
            case 30:
                ...
                posTemp.y += (playerEyeHeight - 0.1f) / 5.229444f * Time.deltaTime;
                if (posTemp.y > CharacterManager.Instance.playerTrans.position.y + playerEyeHeight) {
                    posTemp.y = CharacterManager.Instance.playerTrans.position.y + playerEyeHeight;
                    if (!eyeOpened && elapsedTime >= 5.970185f) {
                        if (CharacterManager.Instance.IsPlayerServal)
                        {
                        ...
                        {
                            CharacterManager.Instance.pCon.SetFaceString("Idle2", 5f);
                        }
                        eyeOpened = true;
                    }
                }
                eventCamPivot[3].position = posTemp;
                if (elapsedTime >= 8.192407f) {
