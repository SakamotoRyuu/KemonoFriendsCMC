            ...
            switch (progress) {
                case 0:
                    if (elapsedTime >= 1f && !achievementChecked) {
                        ...
                        if (isLonesome) {
                            GameManager.Instance.SetSteamAchievement(GameManager.SteamAchievementName.LONESOMESERVAL);
                        } else {
                            GameManager.Instance.SetSteamAchievement(GameManager.SteamAchievementName.GAMECLEAR);
                        }
                    }
                    if (elapsedTime >= 5f) {
                        NextProgress();
