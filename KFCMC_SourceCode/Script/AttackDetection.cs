...
public class AttackDetection : MonoBehaviour {
   ...
   public virtual void WorkEnter(GameObject other) {
    ...
    protected virtual bool HitAttack(ref DamageDetection targetDD) {
        ...
        return SendDamage(ref targetDD, ref closestPoint, ref direction);
    }
    ...
    public virtual void ParticlesClear()
    {
        if (particles != null && particles.Length > 0)
        {
            for (int i = 0; i < particles.Length; i++)
    ...
    public virtual void S_ParticlesClear()
    {
        if (s_particles != null && s_particles.Length > 0)
        {
            for (int i = 0; i < s_particles.Length; i++)
    ...
    public virtual bool IsParticlesEmitting()
    {
        bool answer = false;
        if (particles != null && particles.Length > 0)
        {
