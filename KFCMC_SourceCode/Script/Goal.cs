...
public class Goal : MonoBehaviour {
    ...
    protected virtual void Move()
    {
        if (StageManager.Instance && StageManager.Instance.dungeonMother)
        {
            if (StageManager.Instance.dungeonMother.goldType == DungeonMother.GoldType.MemoryFragment &&
                StageManager.Instance.dungeonController.IsReaperEnabled)
            {
                int bonusNum = StageManager.Instance.dungeonController.ReaperCountRemain / 10;
                if (bonusNum > 0)
                {
                    GameManager.Instance.save.memoryBonus += bonusNum;
                }
            }
            StageManager.Instance.dungeonMother.MoveFloor(floorNum, saveSlot, additive, healDisabled);
    ...
    protected virtual void ActionSubmitButton() {
        ...
        if (StageManager.Instance && StageManager.Instance.dungeonController && StageManager.Instance.dungeonController.keyItemRemain > 0) {
            if (StageManager.Instance.IsWithoutBringItemStage) {
