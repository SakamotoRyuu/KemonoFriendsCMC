...
public class DungeonController : MonoBehaviour {
    ...
    public bool friendsFootstepEnabled;
    public string randomWarpPointTag;
    ...
    [System.NonSerialized]
    public bool healDisabledOnStart;
    ...
    private float reaperCheckIntervalRemain;
    ...
    private bool forceReaper;
    private int reaperCountSave = -1;
    ...
    void Start() {
        ...
        CharacterManager.Instance.CheckLimitSet(true, true, !healDisabledOnStart);
        mapForceCheckFlag = true;
        // Reaper
        if (StageManager.Instance.dungeonMother)
        {
            bool countTextEnabled = StageManager.Instance.dungeonMother.reaperSettings.countTextEnabled && IsReaperEnabled;
            CharacterManager.Instance.SetReaperCanvasEnabled(countTextEnabled);
            if (countTextEnabled)
            {
                CharacterManager.Instance.SetReaperText(StageManager.Instance.dungeonMother.reaperSettings.appearCount.ToString());
                CharacterManager.Instance.SetReaperTextColor(0);
            }
            else
            {
                CharacterManager.Instance.SetReaperText("");
            }
        }
        // 印可状持ちでボスフロアに入ったとき、サンドスターロウを自動で有効化
        if (GameManager.Instance.CanBringInventory() && 
            enemySettings.enabled && isBossFloor && 
            StageManager.Instance.GetSandstarRawEnabled())
        {
            StageManager.Instance.ActivateSandstarRaw();
        }
    }

    public bool IsReaperEnabled
    {
        get
        {
            return (forceReaper || CharacterManager.Instance.GetDifficultyEffect(CharacterManager.DifficultyEffect.Reaper) != 0) &&
            StageManager.Instance.dungeonMother != null &&
            enemySettings.respawnable &&
            !isBossFloor &&
            (StageManager.Instance.GetReaperID() >= 0 || StageManager.Instance.GetReaperPrefab() != null);
        }
    }
    ...
    void Update() {
        ...
        if (!SceneChange.Instance.GetIsProcessing)
        {
            ...
            if (GameManager.Instance.save.memoryBonus > 0)
            {
                if (MessageUI.Instance != null)
                {
                    string bonusGetText = TextManager.Get("STATUS_MEMORY_FRAGMENT") + "×" + GameManager.Instance.save.memoryBonus + TextManager.Get("MESSAGE_GOTITEM");
                    MessageUI.Instance.SetMessage(bonusGetText, 4, MessageUI.panelType_Information, MessageUI.slotType_Normal);
                }
                GameManager.Instance.save.memoryFragment += GameManager.Instance.save.memoryBonus;
                GameManager.Instance.save.memoryBonus = 0;
                if (CharacterManager.Instance)
                {
                    CharacterManager.Instance.CheckMemoryFragmentMax();
                    CharacterManager.Instance.ShowGold(true);
                }
            }
        }
        if (reaperCheckIntervalRemain > 0)
        {
            reaperCheckIntervalRemain -= deltaTimeCache;
        }
        ...
        if (IsReaperEnabled)
        {
            ...
            if (forceReaper)
            {
                condition = StageManager.Instance.dungeonMother.reaperSettings.appearCount * 2;
            }
            else if (StageManager.Instance.stageNumber != StageManager.homeStageId)
            {
                condition = timeCount +
                    CharacterManager.Instance.defeatEnemyCountSSRaw * StageManager.Instance.dungeonMother.reaperSettings.conditionDefeatMultiplier * (GetMinmiBlackEnabled() ? 1f / (float)minmiBlackRate : 1f);
            }
            if (StageManager.Instance.dungeonMother.reaperSettings.countTextEnabled)
            {
                int countNum = Mathf.FloorToInt(StageManager.Instance.dungeonMother.reaperSettings.appearCount - condition);
                if (reaperCountSave != countNum)
                {
                    reaperCountSave = countNum;
                    CharacterManager.Instance.SetReaperText(countNum.ToString());
                }
            }
            if (reaperCheckIntervalRemain <= 0)
            {
                ...
                {
                    case 0:
                        if (condition >= StageManager.Instance.dungeonMother.reaperSettings.firstAlartCount)
                        {
                            if (MessageUI.Instance)
                            {
                                MessageUI.Instance.SetMessage(TextManager.Get(StageManager.Instance.dungeonMother.reaperSettings.alartTextHeader + "00"),
                                    MessageUI.time_Important, MessageUI.panelType_Information, MessageUI.slotType_Caution);
                            }
                            UISE.Instance.Play(UISE.SoundName.caution);
                            reaperProgress = 1;
                            if (StageManager.Instance.dungeonMother.reaperSettings.countTextEnabled)
                            {
                                CharacterManager.Instance.SetReaperTextColor(1);
                            }
                            reaperCheckIntervalRemain = 1;
                        }
                        break;
                    case 1:
                        if (condition >= StageManager.Instance.dungeonMother.reaperSettings.secondAlartCount)
                        {
                            if (MessageUI.Instance)
                            {
                                MessageUI.Instance.SetMessage(TextManager.Get(StageManager.Instance.dungeonMother.reaperSettings.alartTextHeader + "01"),
                                    MessageUI.time_Important, MessageUI.panelType_Information, MessageUI.slotType_Caution);
                            }
                            ...
                            if (StageManager.Instance.dungeonMother.reaperSettings.countTextEnabled)
                            {
                                CharacterManager.Instance.SetReaperTextColor(2);
                            }
                            reaperCheckIntervalRemain = 1;
                        }
                        break;
                    case 2:
                        if (condition >= StageManager.Instance.dungeonMother.reaperSettings.appearCount)
                        {
                            if (MessageUI.Instance)
                            {
                                MessageUI.Instance.SetMessage(TextManager.Get(StageManager.Instance.dungeonMother.reaperSettings.alartTextHeader + "02"),
                                    MessageUI.time_Important, MessageUI.panelType_Information, MessageUI.slotType_Caution);
                            }
                            // リーパー撃破後は元のBGMを再生しないため、BGM番号を変更する
                            ...
                            // リーパー撃破後は元のアンビエントを再生するため、アンビエント番号を変更しない
                            Ambient.Instance.Play(StageManager.Instance.GetReaperAmbient());
                            ...
                            GameObject reaperPrefab = StageManager.Instance.GetReaperPrefab();
                            if (reaperPrefab != null)
                            {
                                Instantiate(reaperPrefab, transform);
                            }
                            if (StageManager.Instance.dungeonMother.reaperSettings.collapseOnAppearEnabled && !collapseUsed)
                            {
                                BreakWalls(false);
                            }
                            reaperProgress = 3;
                            ...
                            if (StageManager.Instance.dungeonMother.reaperSettings.countTextEnabled)
                            {
                                CharacterManager.Instance.SetReaperCanvasEnabled(false);
                            }
                            reaperCheckIntervalRemain = 1;
                        }
                        break;
        ...
        if (PauseController.Instance && !PauseController.Instance.pauseGame) {
            if (disadvantageType != DisadvantageType.None) {
                ...
                if (damageRate > 0f) {
                    ...
                    if (!disadvantageMessageShowed && timeCount > 0 && MessageUI.Instance && !SceneChange.Instance.GetIsProcessing && !PauseController.Instance.pauseGame && PauseController.Instance.GetBlackCurtainAmount <= 0f) {
    ...
    public void Generate() {
        ...
        if (chestSettings.enabled) {
            ...
            if (StageManager.Instance && StageManager.Instance.dungeonMother)
            {
                if (StageManager.Instance.dungeonMother.goldType == DungeonMother.GoldType.Gold)
                {
                ...
                else if (StageManager.Instance.dungeonMother.goldType == DungeonMother.GoldType.MemoryFragment)
                {
    ...
    public Vector3 GetRespawnPos(string targetTag = "Respawn", bool forceUseTag = false) {
        if (tileSettings.enabled && !forceUseTag) {
    ...
    public void GenerateMemoryFragments(float numRate = 1, float yOffsetMin = 0, float yOffsetMax = 0)
    {
        if (chestSettings.container == null)
        {
            chestSettings.container = SetContainer("ChestContainer");
        }
        int num = 20 + Mathf.Clamp(generatorLevel * 5, 0, 100);
        Vector3 offset = vecZero;
        Vector2Int point;
        bool enableOffset = (yOffsetMin != 0f || yOffsetMax != 0f);
        if (num > 0)
        {
            for (int i = 0; i < num; i++)
            {
                ...
                if (point.x >= 0)
                {
                    ...
                    Instantiate(ItemDatabase.Instance.GetItemPrefab((int)ItemDatabase.ItemID.MemoryFragment), 
                        tiles[point.x, point.y].position + offset, quaIden, chestSettings.container);
                }
            }
        }
    }
        ...
        if (numCheck) {
            ...
            if (enemySettings.enemy[enemySettings.pointer].prefab) {
                ...
                if (eBase != null) {
                    ...
                    eBase.SetLevel(enemySettings.enemy[enemySettings.pointer].level + levelBias, false, false, 
                        variableLevel <= 0 ? 0 : Mathf.Clamp(Mathf.Min(variableLevel + levelBias * 10, variableLevel * (levelBias + 1)), 1, CharacterDatabase.Instance.variableStatus.Length));
                }
                if (!fixEnemyPointer) {
                    enemySettings.pointer = (enemySettings.pointer + 1) % enemySettings.enemy.Length;
                }
            ...
            if (enemySettings.enemy[enemySettings.pointer].prefab) {
                ...
                if (eBase != null) {
                    ...
                    if (!eBase.isBoss) {
                        ...
                        eBase.enemyID = enemySettings.enemy[enemySettings.pointer].id;
                        eBase.SetLevel(enemySettings.enemy[enemySettings.pointer].level + levelBias, false, false,
                            variableLevel <= 0 ? 0 : Mathf.Clamp(variableLevel + levelBias * 10, 1, CharacterDatabase.Instance.variableStatus.Length));
                    }
                }
                enemySettings.pointer = (enemySettings.pointer + 1) % enemySettings.enemy.Length;
            }
    ...
    {
        if (enemySettings.enemy.Length > enemySettings.pointer)
        {
            return StageManager.Instance.dungeonController.enemySettings.enemy[enemySettings.pointer].id;
        }
        return -1;
    }

    public void SetForceReaper()
    {
        forceReaper = true;
        reaperCheckIntervalRemain = 0.5f;
    }

    public int ReaperCountRemain
    {
        get
        {
            if (IsReaperEnabled)
            {
                return reaperCountSave;
            }
            else
            {
                return 0;
    ...
    public bool IsRandomWarpPointTagEnabled
    {
        get
        {
            return randomWarpPointTag != "";
        }
    }

    public string GetRandomWarpPointTag
    {
        get
        {
            return randomWarpPointTag;
