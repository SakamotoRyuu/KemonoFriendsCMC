...
public class FollowTarget : MonoBehaviour {
    ...
    public bool useRotateTowards;
    public bool notCountElapsedTimeOnKinematic;
    ...
    protected virtual void FixedUpdate() {
        ...
        if (elapsedTime >= delayTime) {
            if (rb != null && (!disableOnKinematic || !rb.isKinematic)) {
                if (alwaysAddForce || targetObj) {
                    ...
                    {
                        Quaternion rotA = rb.rotation;
                        Quaternion rotB = Quaternion.LookRotation(velocity);
                        if (useRotateTowards)
                        {
                            rb.MoveRotation(Quaternion.RotateTowards(rotA, rotB, lookSpeed * fixedDelta));
                        }
                        else
                        {
                            rb.MoveRotation(Quaternion.Slerp(rotA, rotB, lookSpeed * fixedDelta));
                        }
                    }
                    isTargeting = true;
                    velocity *= speed;
        ...
        if (!notCountElapsedTimeOnKinematic || (rb && !rb.isKinematic))
        {
