...
public class AttackDetectionParticle : AttackDetectionSick {
    ...
    public float particleVelocityThreshold = 0;
    ...
    void OnParticleCollision(GameObject other) {
        if (!isEffectOnly && (parentCBase || independenceOnAwake) && other.CompareTag(targetTag)) {
            if (particleVelocityThreshold > 0)
            {
                int numCollisionEvents = particles[attackParticleIndex].GetCollisionEvents(other, collisionEvents);
                float maxVelocity = 0;
                for (int i = 0; i <  numCollisionEvents; i++)
                {
                    float sqrMagnitude = collisionEvents[i].velocity.sqrMagnitude;
                    if (sqrMagnitude > maxVelocity)
                    {
                        maxVelocity = sqrMagnitude;
                    }
                }
                if (maxVelocity < particleVelocityThreshold * particleVelocityThreshold)
                {
                    return;
                }
            }
