...
public class StageManager : SingletonMonoBehaviour<StageManager> {
    ...
    public const int caveEntranceFloorNumber = 14;
    public const int tribulationsStageNumber = 16;
    ...
    public bool IsTribulationsStage
    {
        get
        {
            return stageNumber == tribulationsStageNumber;
        }
    }

    public bool IsWithoutBringItemStage
    {
        get
        {
            return IsRandomStage || IsTribulationsStage;
        }
    }
    ...
    public bool IsTribulationsBusFloor
    {
        get
        {
            return (stageNumber == homeStageId && floorNumber == amusementParkFloorNumber) ||
                (stageNumber == homeStageId && floorNumber == caveEntranceFloorNumber) ||
                (stageNumber == tribulationsStageNumber);
        }
    }
    ...
    public void MoveStage(int index, int floor = 0, int saveSlot = -1, bool sandstarMax = false) {
        ...
        if (index == homeStageId) {
            ...
            GameManager.Instance.save.memoryFragment = 0;
    ...
    public int GetContainerRank(int pointBias = 0)
    {
        ...
        {
            pointMax = stageNumber * 3 + FloorNumberConsiderDenotative + 1 + pointBias;
        }
    ...
    public GameObject GetReaperPrefab()
    {
        if (dungeonMother)
        {
            return dungeonMother.reaperSettings.reaperPrefab;
        }
        return null;
    }
    ...
    public int GetReaperAmbient()
    {
        if (dungeonMother)
        {
            return dungeonMother.reaperSettings.appearAmbientNumber;
        }
        return -1;
    }
    ...
    public void ActivateSandstarRaw() {
        ...
        if (dungeonController) {
            if (dungeonController.isBossFloor) {
                ...
                dungeonController.SetForceReaper();
            }
    ...
    public void CleaningObjects() {
        ...
        if (AudioSourcePool.Instance) {
            ...
            AudioSourcePool.Instance.Reduce();
        }
        if (ParticleSystemPool.Instance) {
            ParticleSystemPool.Instance.StopAll();
            ParticleSystemPool.Instance.Reduce();
        }
    }
    ...
    public void GenerateDungeon(GameObject dungeonPrefab, int floorNum, bool healDisabled = false) {
        dungeonController = Instantiate(dungeonPrefab, Vector3.zero, Quaternion.identity).GetComponent<DungeonController>();
        if (dungeonController) {
            dungeonController.healDisabledOnStart = healDisabled;
            ...
            // 経験値初期化せずにスカイツリーなどに入った場合、バリアブルレベルを100に変更し、ふしぎな球を削除
            if (GameManager.Instance && GameManager.Instance.save.isExpPreserved == 0)
            {
                if (dungeonController.variableLevel > 0 || GameManager.Instance.CanBringInventory())
                {
                    dungeonController.variableLevel = GameManager.levelMax;
                }
                if (dungeonController.itemSettings.item.Length == 1 && dungeonController.itemSettings.item[0].id == (int)ItemDatabase.ItemID.UnlockConsole)
                {
                    dungeonController.itemSettings.enabled = false;
                }
            }
    ...
    public bool CheckPhotoStageCondition()
    {
        ...
        else if (IsTribulationsStage && GameManager.Instance.GetSecret(GameManager.SecretType.CaveOfTribulationsCleared))
        {
            return true;
        }
        else
    ...
    public bool IsFriendsFootstepEnabled
    {
        get
        {
            return dungeonController && dungeonController.friendsFootstepEnabled;
        }
    }
    ...
    public int GetEnemyLevelMax
    {
        get
        {
            if (dungeonMother)
            {
                return dungeonMother.enemyLevelMax;
            }
            else
            {
                return DungeonMother.defaultEnemyLevelMax;
            }
        }
    }

    public bool IsComboRankEnabled
    {
        get
        {
            if (dungeonMother)
            {
                return dungeonMother.comboRankEnabled;
            }
            else
            {
                return false;
    ...
    {
        if (dungeonMother && dungeonMother.homeSettings.overrideHomeEnabled)
        {
            return dungeonMother.homeSettings.homeFloorNumber;
        }
        return homeFloorNumber;
    }

    public string GetReturnToHomeText(bool isAnother)
    {
        if (dungeonMother && dungeonMother.homeSettings.overrideHomeEnabled)
        {
            return dungeonMother.homeSettings.returnToHomeText;
        }
        else if (isAnother)
        {
            return "CHOICE_QUIT_STAGE_ANOTHER";
        }
        else
        {
            return "CHOICE_QUIT_STAGE";
        }
    }
