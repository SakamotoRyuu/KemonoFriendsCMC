...
public class ItemGetter : MonoBehaviour {
    ...
    */
    public AudioSourcePool.AudioName itemAudio;
    public AudioSourcePool.AudioName itemImportantAudio;
    public AudioSourcePool.AudioName moneyAudio;
    public ParticleSystemPool.ParticleName moneyParticle;
    ...
    const string memoryFragment = "MemoryFragment";
            ...
            switch (PauseController.Instance.ChoicesControl()) {
                ...
                case 0:
                    ...
                    if (getItem && PauseController.Instance.UseItem(getItem.id)) {
                        if (ItemDatabase.IsConsumables(getItem.id))
                        {
    ...
    void GetModify(GetItem getItemTemp) {
        ...
        if (getItemTemp.itemType == GetItem.ItemType.Important) {
            ...
            if (AudioSourcePool.Instance) {
                AudioSourcePool.Instance.Play(itemImportantAudio, trans.position);
            }
        } else {
            ...
            if (AudioSourcePool.Instance) {
                AudioSourcePool.Instance.Play(itemAudio, trans.position);
            }
        }
    ...
    public void GetItemProcess(GetItem getItemTemp) {
        ...
        switch (getItemTemp.itemType) {
            ...
            case GetItem.ItemType.HealSandstar:
                ...
                Destroy(getItemTemp.gameObject);
                break;
            case GetItem.ItemType.ComboStar:
                CharacterManager.Instance.Heal(80, 20, (int)EffectDatabase.id.itemHeal01, true, true, true, true, true);
                CharacterManager.Instance.AddSandstar(1.25f, true, (int)EffectDatabase.id.itemSandstar, true, true);
                CharacterManager.Instance.StaminaHeal(25, true, true);
                if (getItemTemp == getItem)
                {
                    getItem = null;
                }
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        else if (other.CompareTag(goldTag))
        {
            ...
            if (getMoney)
            {
                ...
                ParticleSystemPool.Instance.Play(moneyParticle, trans.position);
                if (!sounded)
                {
                    AudioSourcePool.Instance.Play(moneyAudio, trans.position);
                    sounded = true;
                }
        ...
        else if (other.CompareTag(memoryFragment))
        {
            getMoney = other.gameObject.GetComponent<GetMoney>();
            if (getMoney)
            {
                GameManager.Instance.save.memoryFragment += getMoney.num;
                ParticleSystemPool.Instance.Play(ParticleSystemPool.ParticleName.GetMemoryFragment, trans.position);
                if (!sounded)
                {
                    AudioSourcePool.Instance.Play(AudioSourcePool.AudioName.GetMemoryFragment, trans.position);
                    sounded = true;
                }
                if (CharacterManager.Instance)
                {
