...
public class MessageUI : SingletonMonoBehaviour<MessageUI> {
    ...
    public int SetMessageOptional(string text, Color backColor1, Color backColor2, int twoToneType, int faceId, float time = time_Default, int panelType = 0, int slotType = -1, bool isFirstIn = false) {
        ...
        if (answer >= 0) {
            ...
            if (mesSlot.faceName) {
                mesSlot.faceName.text = ItemDatabase.Instance.GetItemNameNL(faceId);
                if (GameManager.Instance.save.language == GameManager.languageKorean) {
                    mesSlot.faceName.fontSizeMin = 19.85f;
                }
            }
        }
        return answer;
    ...
    public int SetMessageReplace(string text, string messageId, float time = time_Default, int panelType = 0, int slotType = -1, bool isFirstIn = false)
    {
        int hashCode = messageId.GetHashCode();
        for (int i = 0; i < slotMax; i++)
        {
            if (messageSlot[i] && messageSlot[i].hashCode == hashCode)
            {
                DestroySpecificMessage(i);
            }
        }
        int index = SetMessage(text, time, panelType, slotType, isFirstIn);
        if (index >= 0 && messageSlot[index])
        {
            messageSlot[index].hashCode = hashCode;
        }
        return index;
    }
