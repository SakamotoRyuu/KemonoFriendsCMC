...
public class CharacterBase : MonoBehaviour
{
    ...
    public enum SickType
    {
        Mud, // index:0 / bit:1
        Fire, // index:1 / bit:2
        Poison, // index:2 / bit:4
        Ice, // index:3 / bit:8
        Acid, // index:4 / bit:16
        Slow, // index:5 / bit:32
        Stop, // index:6 / bit:64
        Frightened, // index:7 / bit:128
        Radiation // index:8 / bit:256
    };

    public enum FaceName
    {
        Idle, Blink, Idle1, Idle2, Attack, Damage, Dead, Fear, Jump, Run, Refresh, Smile, Other
    }
    ...
    protected bool groundedFlagSave = true;
    ...
    protected int targetSearchAreaIndex = -1;
    ...
    protected float counterDodgeIntervalTimeRemain;
    ...
    public bool IsBuffByPlayer { get; protected set; }
    ...
    protected bool stateJustTransitionedFlag; // stateが遷移したフレーム内であることを示すフラグ
    protected State previousState = State.Spawn; // 遷移直前のstate
    ...
    */
    ...
    protected virtual GameObject SetMapChip()
    {
        return null;
    }
    ...
    protected virtual void Update_TimeCount()
    {
        ...
        if (counterDodgeIntervalTimeRemain > 0f)
        {
            counterDodgeIntervalTimeRemain -= deltaTimeMove;
        }
        ...
        if (knockRecovery > 0f)
        {
            ...
            {
                knockRemain += knockRecovery * GetKnockRecoverySpeedRate() * deltaTimeMove;
                if (knockRemain > heavyEnduranceTemp)
                {
            ...
            {
                knockRemainLight += knockRecovery * GetKnockRecoverySpeedRate() * lightKnockRecoveryRate * deltaTimeMove;
                if (knockRemainLight > lightEnduranceTemp)
                {
    ...
    protected virtual void Update_StatusJudge()
    {
        ...
        else
        {
            ...
            if (agent)
            {
                bool isOnNavMesh = agent.enabled && agent.isOnNavMesh;
                if (isOnNavMesh)
                {
                    groundedFlag_ForAgent = true;
                }
                else
                {
                    groundedFlag_ForAgent = CheckNearNavMesh(tolerance);
                }
                if (state != State.Jump && !(state == State.Attack && isJumpingAttack) && isOnNavMesh && !agent.isOnOffMeshLink)
                {
                    groundedFlag = true;
                }
                else
            ...
            {
                groundedFlag_ForAgent = groundedFlag = CheckGrounded(tolerance);
            }
        }
        if (nowST < 0f)
        {
    ...
    protected virtual float GetTargetDistance(bool isSqrDist = true, bool ignoreY = true, bool considerTargetRadius = false)
    {
        ...
        if (targetTrans)
        {
            ...
            if (ignoreY)
            {
                targetPos.y = transPos.y;
            }
            if (considerTargetRadius)
            {
                if ((targetPos - transPos).sqrMagnitude <= targetRadius * targetRadius)
                {
                    return 0f;
                }
                else
                {
                    targetPos += (transPos - targetPos).normalized * targetRadius;
                }
            }
            if (isSqrDist)
            {
                dist = (targetPos - transPos).sqrMagnitude;
            }
            else
            {
                dist = Vector3.Distance(targetPos, transPos);
            }
        }
        return dist;
    }
    ...
    protected virtual void Update_Sick()
    {
        ...
        CheckSickEffect(SickType.Radiation, (int)EffectDatabase.id.sickRadiation, true);
        ...
        if (GetSick(SickType.Radiation) && sickEffectInterval[(int)SickType.Radiation] <= 0f)
        {
            sickEffectInterval[(int)SickType.Radiation] = 0.5f / CharacterManager.Instance.GetDifficultyEffect(CharacterManager.DifficultyEffect.SlipDamage);
            if (nowHP > 1)
            {
                AddNowHP(Mathf.Min(nowHP - 1, 94) * (-1), GetCenterPosition(), true, colorTypeDamage);
            }
        }
    ...
    protected virtual void Update_Transition_Moves()
    {
        if (!groundedFlag)
        {
            if (stateJustTransitionedFlag && (previousState == State.Attack || previousState == State.Damage || previousState == State.Dodge))
            {
                // 上記ステートから同一フレーム内でJumpステートへ遷移する場合は、Y速度をリセットしない
            }
            else
            {
                move.y = 0;
    ...
    protected virtual void Update_MoveControl()
    {
        ...
        if (deltaTimeMove > 0f)
        {
            Vector3 moveSum = move * deltaTimeMove + moveDelta;
            if (moveSum != vecZero)
            {
                cCon.Move(moveSum);
            }
    ...
    protected virtual void Update()
    {
        if (CharacterManager.Instance)
        {
            stateJustTransitionedFlag = false;
            Update_GetDelta();
            Update_StatusJudge();
            Update_Sick();
            Update_Targeting();
    ...
    protected virtual void SetComboRankAttackPoint(int type, float cost, int weight)
    {
    }

    protected virtual void ConsumeStaminaForAttack(int type, float cost)
    {
        nowST -= cost;
        if (nowST < 0f)
        {
    ...
    protected bool AttackBase(int type, float pMul, float kMul, float cost = 0, float stiff = 0.5f, float interval = 0.5f, float moveSpeedMultiplier = 1, float animSpeedMultiplier = 1, bool lockonOnStart = true, float lockonSpeed = -1)
    {
        if (!isPlayer || JudgeStamina(cost))
        {
            ...
            if (consumeStamina)
            {
                ConsumeStaminaForAttack(type, cost);
            }
    ...
    protected void SetState(State targetState)
    {
        if (state != State.Dead)
        {
            ...
            stateJustTransitionedFlag = true;
            stateTime = 0f;
            state = targetState;
            ChangeStateReset();
            StartState();
    ...
    public void AddNowST(float percent)
    {
        float maxSTTemp = GetMaxST();
        nowST += maxSTTemp * percent / 100;
        if (nowST > maxSTTemp)
        {
            nowST = maxSTTemp;
        }
    ...
    public virtual void TakeDamage(int damage, Vector3 position, float knockAmount, Vector3 knockVector, CharacterBase attacker = null, int colorType = 0, bool penetrate = false)
    {
        if (GetCanTakeDamage(penetrate))
        {
            ...
            // 衝撃力値を計算（残HPの影響を受けることがあるためHP減算後におこなう）
    ...
    {
        if (counterDodgeIntervalTimeRemain <= 0)
        {
            counterDodgeIntervalTimeRemain = dodgeStiffTime;
    ...
    public virtual void SpecialStep(float distanceToTarget, float stepTime = 0.25f, float maxDist = 4f, float minDist = 0f, float defaultDist = 0f, bool ignoreY = true, bool considerTargetRadius = true, EasingType easingType = EasingType.SineInOut, bool directionAdjustEnabled = false)
    {
        ...
        {
            targetPos = transPos + trans.TransformDirection(vecForward);
        }
        if (ignoreY)
        {
            targetPos.y = transPos.y;
        ...
        if (dist > 0f)
        {
            SetSpecialMove((targetPos - transPos).normalized, dist, stepTime, easingType, null, directionAdjustEnabled);
        }
    }
    ...
    public virtual void ApproachOrSeparateBetween(float approachDistance, float separateDistance)
    {
        if (targetTrans)
        {
            float dist = GetTargetDistance(false, fbStepIgnoreY, fbStepConsiderRadius);
            ...
            {
                SetSpecialMove(GetTargetVector(true, true, false), Mathf.Min(dist - approachDistance, fbStepMaxDist * speedRate), fbStepTime, fbStepEaseType);
            }
            else if (dist < separateDistance)
            {
                SetSpecialMove(GetTargetVector(true, true, true), Mathf.Min(separateDistance - dist, fbStepMaxDist * speedRate), fbStepTime, fbStepEaseType);
            }
        }
    }
    ...
    protected void Update_SpecialMove()
    {
        if (specialMoveElapsedTime < specialMoveDuration && specialMoveDuration > 0f && deltaTimeMove > 0f)
        {
            ...
            {
                moveDelta += specialMoveVectorTemp;
            }
    ...
    public virtual void AnimationStopTiming()
    {
        isAnimStopped = true;
    }
    public virtual void AnimationStopCancel()
    {
        isAnimStopped = false;
    }
    ...
    public void AgentOffYieldOn()
    {
        StartCoroutine(AgentOffYieldOnCoroutine());
    }

    IEnumerator AgentOffYieldOnCoroutine()
    {
        if (agent && agent.enabled)
        {
            agent.enabled = false;
    ...
    {
        int sickIndex = (int)sickType;
        if (duration > 0 && sickRemainTime[sickIndex] <= 0)
        {
            sickEffectInterval[sickIndex] = Mathf.Clamp(sickEffectInterval[sickIndex], 0f, 2f);
        }
        if (sickType == SickType.Stop && duration > 0)
        {
            specialMoveDuration = 0f;
        }
        if (duration <= 0 || duration > sickRemainTime[sickIndex])
        {
            sickRemainTime[sickIndex] = duration;
        }
    ...
    public void RandomWarp(Vector3 offset, bool nearGoal = false, string targetTag = "Respawn", float disableControlTime = 0f, float gravityZeroTime = 0f, bool forceUseTag = false)
    {
        if (StageManager.Instance && StageManager.Instance.dungeonController)
        {
            Vector3 warpPos = StageManager.Instance.dungeonController.GetRespawnPos(targetTag, forceUseTag);
            /*
            if (searchFar) {
                Vector3 pivotPos = trans.position;
            ...
            */
            if (nearGoal)
            {
                ...
                if (goalInstance)
                {
                    ...
                    for (int i = 0; i < 15; i++)
                    {
                        Vector3 tempPos = StageManager.Instance.dungeonController.GetRespawnPos(targetTag, forceUseTag);
    ...
    public virtual void BlownAway(bool disableControl = true, float height = 5f, bool nearGoal = false, string targetTag = "Respawn")
    {
        ...
        move.y = 0f;
        if (StageManager.Instance && StageManager.Instance.dungeonController.IsRandomWarpPointTagEnabled)
        {
            RandomWarp(vecUp * height, nearGoal, StageManager.Instance.dungeonController.randomWarpPointTag, disableControlTimeRemain, gravityZeroTimeRemain, true);
        }
        else
        {
    ...
    protected void Continuous_Approach(float baseSpeed = 10f, float brakeDistance = 0.5f, float stopDistance = 0.01f, bool ignoreY = true, bool considerRadius = false, float brakingSpeedRate = 0.5f)
    {
        if (targetTrans && deltaTimeMove > 0f && !GetSick(SickType.Stop))
        {
            ...
            if (sqrDist > stopDistance)
            {
                ...
                move += GetTargetVector(true, true) * speed;
            }
        }
    }

    protected void ApproachTransformPivot(Transform pivot, float baseSpeed = 10f, float brakeDistance = 0.5f, float stopDistance = 0.01f, bool ignoreY = true, float brakingSpeedRate = -1f)
    {
        if (targetTrans && pivot && deltaTimeMove > 0f && !GetSick(SickType.Stop))
        {
            ...
            if (sqrDist > stopDistance * stopDistance)
            {
                ...
                else if (!GetSick(SickType.Slow) && brakeStopDiff > 0f && sqrDist < brakeDistance * brakeDistance)
                {
                    ...
                    if (brakingSpeedRate < 0f)
                    {
                        brakingSpeedRate = isEnemy ? 0.2f : 0.5f;
                    }
                    speed *= Mathf.Lerp(brakingSpeedRate, 1f, (dist - stopDistance) / brakeStopDiff);
                    if (isEnemy && speed > dist * 60f)
                    {
                        speed = dist * 60f;
                    }
                }
                // cCon.Move((targetPosTemp - pivotPosTemp).normalized * speed * deltaTimeMove);
                move += (targetPosTemp - pivotPosTemp).normalized * speed;
            }
        }
    }
    ...
    public virtual void RemoveRide(bool replaceToReleasePoint = false)
    {
        if (rideTarget && !isEnemy && AnimHash.Instance)
        {
            ...
            ReleaseWarpStan();
            Vector3 vecTemp = trans.localEulerAngles;
            vecTemp.x = 0f;
            vecTemp.z = 0f;
            trans.localEulerAngles = vecTemp;
    ...
    public virtual void Attraction(GameObject decoy, AttractionType type, bool lockForce = false, float targetFixingTime = 0f, bool isAttracterPlayer = false) { }
    ...
    protected virtual void SkipHeavyKnockAnimation() { }

    // Knockゲージ関連
    public virtual float GetKnockGaugeNow()
    {
        if (state == State.Dead)
        {
            return 0;
        }
        else if (state == State.Damage && isDamageHeavy)
        {
            return 0;
        }
        else
    ...
    public virtual float GetKnockGaugeMax()
    {
        return knockEndurance;
    }

    public virtual bool KnockRecoveryGaugeEnabled()
    {
        return state == State.Damage && isDamageHeavy;
    }

    public virtual bool ChimeraGaugeEnabled()
    {
        return false;
    }

    public virtual float GetChimeraGaugeRate()
    {
        return 0;
    }

    /* Knock回復ゲージの表示上のレートを取得 */
    public virtual float GetKnockRecoveryGaugeRate()
    {
        if (state == State.Damage && isDamageHeavy)
        {
            float stiffTime = GetHeavyStiffTime();
            if (stiffTime > 0)
            {
                return Mathf.Clamp01(stateTime / stiffTime);
            }
        }
        return 0;
    }
    ...
    /* Knock回復速度レートを取得 */
    protected virtual float GetKnockRecoverySpeedRate()
    {
        return 1;
    }

    /* ガッツ残存率を0から1の間で返す */
    public virtual float GetGutsRate()
    {
        return 0;
    }

    /*  */
    public float GetTargetUpperAngle(Transform anglePivot, bool downerEnabled = false)
    {
        if (targetTrans)
        {
            Vector3 targetPosition = targetTrans.position;
            Vector3 pivotPosition = anglePivot.position;
            if (downerEnabled || targetPosition.y > pivotPosition.y)
            {
                Vector3 ignoreYPosition = targetPosition;
                ignoreYPosition.y = pivotPosition.y;
                Vector3 ignoreYVector = ignoreYPosition - pivotPosition;
                Vector3 targetVector = targetPosition - pivotPosition;
                if (ignoreYVector == targetVector)
                {
                    return 0;
                }
                else if (ignoreYVector == vecZero)
                {
                    if (targetPosition.y < pivotPosition.y)
                    {
                        return -90;
                    }
                    else
                ...
                float angle = Vector3.Angle(ignoreYVector, targetVector);
                if (targetPosition.y < pivotPosition.y)
                {
                    angle *= -1;
                }
    ...
    /// <summary>
    /// 攻撃ヒット時の通知をAttackDetectionNoticeToParentから受け取る
    /// </summary>
    public virtual void ReceiveNoticeForHitAttack(DamageDetection damageReceiver)
    {
    }

    public float GetPredictedPositionY(float seconds)
    {
        if (groundedFlag)
        {
        ...
        {
            return trans.position.y + move.y * seconds + Physics.gravity.y * 0.5f * seconds * seconds;
        }
    }

    public virtual void AddComboPoint(int colorType)
    {
    }
