...
public class DicFriendsConsole : ConsoleBase {
    ...
    public class EnemyData {
        ...
        public bool faceExpressionEnabled;
        public MotionException[] motionExceptions;
    }
    ...
    public Color[] enemyLevelColor; // GameManager.enemyLevelMax * 2 + 1まで確保
    ...
    public int enemyMaxNonSecret;
    public int itemMaxNonSecret;
    ...
    private int facilityMax;
    private int itemMax;

    private static readonly string[] enemyColorTags = new string[] {
        "<color=#FFFFBF>", "<color=#BFDFFF>", "<color=#FFC0C0>", "<color=#DFBFFF>", "<color=#C0C0C0>", "<color=#BFFFD4>",
        "<color=#BFFFFF>", "<color=#BFFFBF>", "<color=#FFCFBF>", "<color=#BFBFFF>", "<color=#C0C0C0>", "<color=#FFFFFF>"
    };
    ...
    private void SetFriends(int pointer) {
        ...
        if (pointer >= 0 && pointer < friendsMax) {
            ...
            if (characterPrefab) {
                if (CheckFriendsCondition(pointer)) {
                    ...
                    if (friendsData[pointer].isNPC) {
                        ...
                        if (fBase) {
                            fBase.SetForDictionary(cameraObj.transform);
                        }
                    }
                    ...
                    if (wildReleaseFlag && fBase && GetEnableWildRelease()) {
                        ...
                        fBase.DestroySupermanDuringEffectObj();
                    }
                    if (cameraObj) {
    ...
    private void SetEnemyLevel(int pointer) {
        if (eBase) {
            ...
            if (enemyData[pointer].type == EnemyType.Boss) {
                eBase.SetForDictionary(enemyData[pointer].defaultSuperman || enemyLevelNow >= GameManager.enemySinWRLevel, friendsLayerObj.layer, cameraObj.transform);
            }
        }
        SetTalkBack(enemyMessageColor[(enemyData[pointer].type == EnemyType.Boss ? GameManager.enemyLevelMax : 0) + enemyLevelNow]);
    }
        ...
        if (pointer >= 0 && pointer < enemyMax) {
            ...
            if (characterPrefab) {
                if (CheckEnemyCondition(pointer)) {
                    ...
                    for (int i = 0; i < infoArray.Length; i++) {
                        if (!string.IsNullOrEmpty(infoArray[i])) {
                            if (i == 0) {
                                ...
                                sb.Append(enemyColorTags[(enemyData[pointer].type == EnemyType.ZakoGreen ? GameManager.enemyLevelMax : 0) + i - 1]).Append(infoArray[i]).Append(colorTagEnd);
                            } else {
                                sb.Append(enemyColorTags[(enemyData[pointer].type == EnemyType.ZakoGreen ? GameManager.enemyLevelMax : 0) + i - 1]).AppendLine().Append(TextManager.Get("DIC_ENEMY_INFO_LOCK")).Append(colorTagEnd);
                            }
                        }
                    }
                    ...
                    for (int i = 0; i < GameManager.enemyLevelMax && lvIndex < enemyLevelFill.Length; i++) {
                        if (CharacterDatabase.Instance.enemy[idTemp].statusExist[i] && lvIndex < enemyLevelFill.Length) {
                            ...
                            if (enemyData[pointer].type == EnemyType.ZakoBlue) {
                                ...
                                enemyLevelFill[lvIndex].color = enemyLevelColor[GameManager.enemyLevelMax + i];
                            } else {
                                enemyLevelFill[lvIndex].color = enemyLevelColor[i < 4 ? GameManager.enemyLevelMax * 2 : GameManager.enemyLevelMax * 2 + 1];
                            }
                    ...
                    eBase.SetForItem();
                    anim = characterInstance.GetComponent<Animator>();
                    if (enemyData[pointer].faceExpressionEnabled)
                    {
                        faceCon = characterInstance.GetComponent<FaciemController>();
                    }
    ...
    private void SetFacility(int pointer) {
        ...
        if (pointer >= 0 && pointer < facilityMax) {
                            ...
                            if (idTemp >= 0 && idTemp < GameManager.enemyMax) {
                                ...
                                if (defeatedFlag) {
                                    if (typeTemp != EnemyType.Boss) {
                                        sb.Append(enemyColorTags[(typeTemp == EnemyType.ZakoGreen ? GameManager.enemyLevelMax : 0) + Mathf.Clamp(levelMin, 0, 4)]);
                                    }
    ...
    private bool CheckItemCondition(int pointer) {
        if (pointer >= 0 && pointer < itemMax) {
            int arrayNum = itemData[pointer].id / GameManager.bitUnit;
            int bitNum = 1 << (itemData[pointer].id % GameManager.bitUnit);
    ...
    protected override void ConsoleStart() {
        ...
        if (GameManager.Instance) {
            if (GameManager.Instance.GetSecret(GameManager.SecretType.CaveOfTribulationsCleared))
            {
                enemyMax = enemyData.Length;
                itemMax = itemData.Length;
            }
            else
            {
                enemyMax = enemyMaxNonSecret;
                itemMax = itemMaxNonSecret;
            }
            friendsMax = friendsData.Length;
            facilityMax = facilityData.Length;
            if (GameManager.Instance.save.defeatEnemy.Length < enemyMax * enemyLevelMax)
            {
                GameManager.Instance.FixSaveDefeatEnemyArray();
            }
            switch (choiceAnswer) {
    ...
    protected override void ConsoleUpdate() {
        if (dicState > 0 && GameManager.Instance) {
            ...
            if ((eventEnterNum >= 100 && eventEnterNum < 100 + indexNames.Length) || (eventClickNum >= 100 && eventClickNum < 100 + indexNames.Length)) {
                ...
                switch (dicType) {
                    case 0:
                        pointer = (listOrigin + cursorPos) % friendsMax;
                        SetFriendsChoosingIcon();
                        break;
                    case 1:
                        pointer = (listOrigin + cursorPos) % enemyMax;
                        SetEnemyChoosingIcon();
                        break;
                    case 2:
                        pointer = (listOrigin + cursorPos) % facilityMax;
                        SetFacilityChoosingIcon();
                        break;
                    case 3:
                        pointer = (listOrigin + cursorPos) % itemMax;
                        SetItemChoosingIcon();
                        break;
                }
            }
            if (eventClickNum == 200 || eventClickNum == 201 || mouseScroll != 0) {
                ...
                switch (dicType) {
                    case 0:
                        listOrigin = (listOrigin + moveTemp + friendsMax) % friendsMax;
                        pointer = (listOrigin + cursorPos) % friendsMax;
                        SetFriendsIndex();
                        if (dicState == 2 || dicState == 3) {
                            SetFriends(pointer);
                        }
                        break;
                    case 1:
                        listOrigin = (listOrigin + moveTemp + enemyMax) % enemyMax;
                        pointer = (listOrigin + cursorPos) % enemyMax;
                        SetEnemyIndex();
                        if (dicState == 12 || dicState == 13) {
                            SetEnemy(pointer);
                        }
                        break;
                    case 2:
                        listOrigin = (listOrigin + moveTemp + facilityMax) % facilityMax;
                        pointer = (listOrigin + cursorPos) % facilityMax;
                        SetFacilityIndex();
                        if (dicState == 22 || dicState == 23) {
                            SetFacility(pointer);
                        }
                        break;
                    case 3:
                        listOrigin = (listOrigin + moveTemp + itemMax) % itemMax;
                        pointer = (listOrigin + cursorPos) % itemMax;
                        SetItemIndex();
                        if (dicState == 32 || dicState == 33) {
                            SetItem(pointer);
                        }
                        break;
                }
            ...
            if (move.y != 0) {
                switch (dicState) {
                    ...
                    case 2:
                        UISE.Instance.Play(UISE.SoundName.move);
                        pointer = (pointer + friendsMax + move.y) % friendsMax;
                        if (move.y < 0 && cursorPos <= 0) {
                            listOrigin = pointer;
                            SetFriendsIndex();
                        } else if (move.y > 0 && cursorPos >= indexNames.Length - 1) {
                            listOrigin = (pointer - (indexNames.Length - 1) + friendsMax) % friendsMax;
                            SetFriendsIndex();
                        } else {
                            cursorPos = (pointer - listOrigin + friendsMax) % friendsMax;
                            SetFriendsChoosingIcon();
                        }
                        cursor.anchoredPosition = cursorOrigin + cursorInterval * cursorPos;
                        break;
                    ...
                    case 12:
                        UISE.Instance.Play(UISE.SoundName.move);
                        pointer = (pointer + enemyMax + move.y) % enemyMax;
                        if (move.y < 0 && cursorPos <= 0) {
                            listOrigin = pointer;
                            SetEnemyIndex();
                        } else if (move.y > 0 && cursorPos >= indexNames.Length - 1) {
                            listOrigin = (pointer - (indexNames.Length - 1) + enemyMax) % enemyMax;
                            SetEnemyIndex();
                        } else {
                            cursorPos = (pointer - listOrigin + enemyMax) % enemyMax;
                            SetEnemyChoosingIcon();
                        }
                        cursor.anchoredPosition = cursorOrigin + cursorInterval * cursorPos;
                        break;
                    ...
                    case 22:
                        UISE.Instance.Play(UISE.SoundName.move);
                        pointer = (pointer + facilityMax + move.y) % facilityMax;
                        if (move.y < 0 && cursorPos <= 0) {
                            listOrigin = pointer;
                            SetFacilityIndex();
                        } else if (move.y > 0 && cursorPos >= indexNames.Length - 1) {
                            listOrigin = (pointer - (indexNames.Length - 1) + facilityMax) % facilityMax;
                            SetFacilityIndex();
                        } else {
                            cursorPos = (pointer - listOrigin + facilityMax) % facilityMax;
                            SetFacilityChoosingIcon();
                        }
                        cursor.anchoredPosition = cursorOrigin + cursorInterval * cursorPos;
                        break;
                    ...
                    case 32:
                        UISE.Instance.Play(UISE.SoundName.move);
                        pointer = (pointer + itemMax + move.y) % itemMax;
                        if (move.y < 0 && cursorPos <= 0) {
                            listOrigin = pointer;
                            SetItemIndex();
                        } else if (move.y > 0 && cursorPos >= indexNames.Length - 1) {
                            listOrigin = (pointer - (indexNames.Length - 1) + itemMax) % itemMax;
                            SetItemIndex();
                        } else {
                            cursorPos = (pointer - listOrigin + itemMax) % itemMax;
                            SetItemChoosingIcon();
                        }
                        cursor.anchoredPosition = cursorOrigin + cursorInterval * cursorPos;
                        break;
    ...
    private void SetMotion_Start() {
        ...
        else if (choiceAnswer == choiceType_Enemy && enemyData[pointer].faceExpressionEnabled)
        {
            SetExpressionAccordingMotion();
        }
    }
    ...
    private void SetExpressionAccordingMotion()
    {
        if (faceCon)
        {
            ...
            {
                for (int i = 0; i < enemyData[pointer].motionExceptions.Length; i++)
                {
                    if (detailMotionType == enemyData[pointer].motionExceptions[i].motionIndex && !string.IsNullOrEmpty(enemyData[pointer].motionExceptions[i].faceName))
                    {
                        faceCon.SetFace(enemyData[pointer].motionExceptions[i].faceName);
                        exceptionFound = true;
                        break;
                    }
                }
