...
public class FriendsBase : CharacterBase {
    ...
    protected GameObject supermanDuringEffectObj;
    ...
    protected override void OnEnable() {
        ...
        if (!isItem)
        {
            SetFootstepDetectionActive(isPlayer || (StageManager.Instance && StageManager.Instance.IsFriendsFootstepEnabled));
        }
    }
    ...
    protected override void Awake() {
        ...
        if (animatorForBattle) {
            ...
            superHealHPBias = (maxHP > CharacterManager.maxHPBase ? (float)maxHP / CharacterManager.maxHPBase : 1f);
            maxHPLevel = maxHP * CharacterManager.maxHPLevelRate;
    ...
    */
    ...
    public void HealGuts(float rate)
    {
        gutsRemain += gutsMax * rate;
        if (gutsRemain > gutsMax)
        {
            gutsRemain = gutsMax;
        }
    }
    ...
    */
    ...
    public override float GetGutsRate()
    {
        if (gutsRemain <= 0f || gutsMax <= 0f)
        {
            return 0;
        ...
        {
            return Mathf.Clamp01(gutsRemain / gutsMax);
        }
    }
    ...
    protected override GameObject SetMapChip() {
        GameObject mapChipObj = Instantiate(MapDatabase.Instance.prefab[MapDatabase.friends], trans);
    ...
    protected override void Update_StatusJudge() {
        ...
        if (drowningRemain > 0f)
        {
            ...
            {
                // 水に落ちている間、火の状態異常を回復する
                if (!drownIsLava && !drownIsFalling && GetSick(SickType.Fire))
                {
                    SetSick(SickType.Fire, 0);
                }
            }
        }
        ...
        {
            if (!(!isPlayer && !rideTarget && StageManager.Instance && !StageManager.Instance.IsFriendsFootstepEnabled))
            {
                SetFootstepDetectionActive(!rideTarget);
            }
        }
    }
    ...
    protected override void Update_Sick() {
        ...
        if (state != State.Dead) {
            if (CharacterManager.Instance.GetBuff(CharacterManager.BuffType.Antidote))
            {
                ...
                if (GetSick(SickType.Radiation))
                {
                    SetSick(SickType.Radiation, 0);
                }
            }
            CheckStealth();
    ...
    public override void SetSick(SickType sickType, float duration, AttackDetection attacker = null) {
        ...
        if ((sickType == SickType.Poison || sickType == SickType.Acid || sickType == SickType.Slow || sickType == SickType.Radiation) && 
            CharacterManager.Instance.GetBuff(CharacterManager.BuffType.Antidote)) {
            duration = 0f;
        }
    ...
    protected override void Update_TimeCount() {
        ...
        if (isSuperman) {
            superHealDecimal += 
                CharacterManager.Instance.GetStandardMaxHP() * 
                CharacterManager.Instance.GetSuperHealHyper() * 
                CharacterManager.Instance.GetFriendsEffect(CharacterManager.FriendsEffect.WRHeal) * 
                superHealHPBias * superHealRate * deltaTimeCache;
        ...
        if (mutekiTimeRemain > 0f && !IsInvincibilityEnabled()) {
            mutekiTimeRemain = 0f;
        }
    ...
    public override void TakeDamage(int damage, Vector3 position, float knockAmount, Vector3 knockVector, CharacterBase attacker = null, int colorType = 0, bool penetrate = false) {
        if (eventMutekiTimeRemain <= 0f) {
            ...
            if (gutsRemain > 0f && !IsHellMode() && GameManager.Instance.save.config[GameManager.Save.configID_DisableGuts] == 0) {
    ...
    public override void SetForItem() {
        ...
        SetFootstepDetectionActive(false);
        base.SetForItem();
    }

    public void SetForDictionary(Transform dynamicBoneReferenceObject) {
        SetForItem();
        if (cloth) {
            SetClothEnabled(GameManager.Instance.save.config[GameManager.Save.configID_ClothSimulation]);
        }
        ...
        if (existDynamicBone)
        {
            ...
            {
                dynamicBone[i].m_ReferenceObject = dynamicBoneReferenceObject;
            }
        }
    }
    ...
    public override void SpecialStep(float distanceToTarget, float stepTime = 0.25f, float maxDist = 4, float minDist = 0, float defaultDist = 0, bool ignoreY = true, bool considerTargetRadius = true, EasingType easingType = EasingType.SineInOut, bool directionAdjustEnabled = false) {
        ...
        if (targetTrans) {
            ...
            if (considerTargetRadius && Physics.BoxCast(transPos + specialStepBoxCenterOffset, specialStepBoxHalfExtents, (targetPos - transPos).normalized, out raycastHit, Quaternion.Euler(trans.TransformDirection(vecForward)), maxDist + distanceToTarget, specialStepBoxLayerMask, QueryTriggerInteraction.Ignore)) {
                dist = Mathf.Clamp(raycastHit.distance - distanceToTarget, minDist, maxDist);
            } else {
                dist = Mathf.Clamp(Vector3.Distance(targetPos, transPos) - distanceToTarget, minDist, maxDist);
            }
    ...
    public virtual void SetFieldBuff(FieldBuffType type, bool byPlayer) {
    ...
    public float GetAttackStandard()
    {
        return 10f * GameManager.Instance.GetLevelStatus(LevelForStatus) * (1f + CharacterManager.Instance.GetEquipEffect(CharacterManager.EquipEffect.Attack));
    }
    ...
    public override int GetMaxHP() {
        return IsHellMode() ? 1 : Mathf.RoundToInt(GetMaxHPNoEffected() * (1f + CharacterManager.Instance.GetFriendsEffect(CharacterManager.FriendsEffect.MaxHP)));
    }
    public override int GetMaxHPNoEffected() {
        return IsHellMode() ? 1 : maxHP + Mathf.RoundToInt(maxHPLevel * LevelForStatus) + CharacterManager.Instance.GetMaxHPPlus() * maxHP / 10;
    }
    ...
    public void SetDynamicBoneReferenceObject()
    {
        if (existDynamicBone)
        {
            for (int i = 0; i < dynamicBone.Length; i++)
            {
                ...
                {
                    CameraManager.Instance.SetMainCameraTransform(ref dynamicBone[i].m_ReferenceObject);
                }
                else
                {
                    ...
                    if (camTemp)
                    {
                        dynamicBone[i].m_ReferenceObject = camTemp.transform;
                    }
                }
            }
        }
    ...
    public override void SupermanStart(bool effectEnable = true) {
        if (!isSuperman)
        {
            ...
            if (isEnemy || isPlayer || GameManager.Instance.save.config[GameManager.Save.configID_ObscureFriends] < 3)
            {
                ...
                if (!supermanDuringEffectObj)
                {
                    supermanDuringEffectObj = CharacterManager.Instance.EmitEffect((int)EffectDatabase.id.friendsYKDuring, friendsId, true, false, friendsId >= 0);
                }
                if (supermanDuringEffectObj && friendsId >= 0)
                {
                    ActivateAndPositionToRaycastHit raycasterTemp = supermanDuringEffectObj.GetComponent<ActivateAndPositionToRaycastHit>();
                    if (raycasterTemp)
                    {
    ...
    protected bool IsInvincibilityEnabled()
    {
        bool result = true;
        switch (GameManager.Instance.save.config[GameManager.Save.configID_DisableInvincibility])
        {
            case (int)ConfigConsts.DisableInvincibility.Off:
                result = true;
                break;
            case (int)ConfigConsts.DisableInvincibility.On:
                result = false;
                break;
            case (int)ConfigConsts.DisableInvincibility.OnlyServal:
                result = !isPlayer;
                break;
            case (int)ConfigConsts.DisableInvincibility.OnlyWhenAttacked:
                result = (state != State.Damage && state != State.Dead);
                break;
            default:
                result = true;
                break;
    ...
    public override bool GetCanTakeDamage(bool penetrate = false)
    {
        return base.GetCanTakeDamage(penetrate) || !IsInvincibilityEnabled();
    }
    ...
    protected virtual void ScrewStartSecond() { }
    ...
    protected bool IsHellMode()
    {
        if (CharacterManager.Instance)
        {
            return CharacterManager.Instance.IsHellMode(isPlayer);
        }
        else
        {
            return false;
    ...
    public void SetFootstepDetectionActive(bool active)
    {
        for (int i = 0; i < footstepDetectionObj.Length; i++)
        {
            if (footstepDetectionObj[i] && footstepDetectionObj[i].activeSelf != active)
            {
                footstepDetectionObj[i].SetActive(active);
            }
        }
    }

    protected void RegisterHate()
    {
        ...
        for (int i = 0; i < enemies.Length; i++)
        {
            ...
            if (enemyBaseTemp)
            {
                ...
                else if ((enemies[i].transform.position - trans.position).sqrMagnitude <= 10f * 10f)
                {
    ...
    public void DestroySupermanDuringEffectObj()
    {
        if (supermanDuringEffectObj)
        {
            Destroy(supermanDuringEffectObj);
        }
    }

    public override void HitAttackAdditiveProcess()
    {
        if (antimatterPivot && antimatterPrefab && CharacterManager.Instance.GetEquipEffect(CharacterManager.EquipEffect.Antimatter) != 0)
        {
            for (int i = 0; i < 7; i++)
            {
