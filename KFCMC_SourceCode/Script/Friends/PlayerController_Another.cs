...
public class PlayerController_Another : PlayerController_Serval
{

    public GameObject cervalBall;
    protected const int effectPileSuper = 6;
    ...
    protected override float GetSpinCost()
    {
        return GetCost(CostType.Skill) * 1.5625f; // 25
    }

    float GetSuperSpinCost()
    {
        return GetCost(CostType.Skill) * 2.5f; // 40
    }

    protected override float GetWaveCost()
    {
        return GetCost(CostType.Skill) * 1.25f; // 20
    }
    ...
    protected override void AttackBody() {
        ...
        if (!groundedFlag) {
            if ((CharacterManager.Instance.GetEquipEffect(CharacterManager.EquipEffect.Bolt) != 0 || (isSuperman && CharacterManager.Instance.GetEquipEffect(CharacterManager.EquipEffect.Judgement) != 0)) && (playerInput.GetButton(RewiredConsts.Action.Special) || forceBolt) && JudgeStamina(GetBoltCost())) {
                ...
                if (AttackBase(boltAttackIndex, 1, 1, GetBoltCost(), 5f, 15f / 30f / spRate, 0.2f, 1, lockonEnabled, 20f)) {
                    S_ParticlePlay(0);
                    attackProcess = 1;
                    attackBiasValue = 0;
                    ...
                    SetComboRankAttackPoint(boltAttackIndex, comboPointBolt);
                }
            } else {
                SetAttackFaceForNormal();
                if (AttackBase(jumpAttackIndex, 1, 2, GetCost(CostType.Attack), 5f, 15f / 30f / spRate, 0.2f, 1, lockonEnabled, 20f)) {
                    S_ParticlePlay(0);
                    attackProcess = 1;
                    attackBiasValue = 0;
                    ...
                    SetComboRankAttackPoint(jumpAttackIndex, comboPointJump);
                }
            }
        } else {
            if (CharacterManager.Instance.GetEquipEffect(CharacterManager.EquipEffect.Pile) != 0 && ((playerInput.GetButton(RewiredConsts.Action.Special) && axisInput.y > 0.5f) || forcePile) && JudgeStamina(GetPileCost())) {
                ...
                float spinSp = 3f / 4f;
                SetAttackFaceForSkill();
                bool superSpinEnabled = (isSuperman && nowST >= GetSuperSpinCost());
                float spinInterval = (superSpinEnabled ? 50f : 20f) / 30f / spinSp;
                if (AttackBase(superSpinEnabled ? superSpinAttackIndex : spinAttackIndex, 1f, 1f, superSpinEnabled ? GetSuperSpinCost() : GetSpinCost(), spinInterval, spinInterval, 0.5f, spinSp, false)) {
                    S_ParticlePlay(0);
                    S_ParticlePlay(1);
                    ...
                    SetComboRankAttackPoint(superSpinEnabled ? superSpinAttackIndex : spinAttackIndex, superSpinEnabled ? comboPointSpin * 2 : comboPointSpin);
                }
