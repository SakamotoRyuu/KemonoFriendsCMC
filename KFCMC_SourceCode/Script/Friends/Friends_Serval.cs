...
public class Friends_Serval : FriendsBase
{
    ...
    protected float maxSTBase;
    protected float attackPowerBase;
    protected float knockPowerBase;
    protected float dodgePowerBase;
    ...
    protected override void Awake() {
        ...
        if (animatorForBattle) {
            ...
            maxSTBase = maxST;
            attackPowerBase = attackPower;
            knockPowerBase = knockPower;
            dodgePowerBase = dodgePower;
        }
    }
    ...
    float GetWaveCost()
    {
        return GetCost(CostType.Skill) * (isPlasma ? 1.25f : 1.0f);
    }
    ...
    {
        isPlasma = (CharacterManager.Instance.GetEquipEffect(CharacterManager.EquipEffect.Plasma) != 0) && GameManager.Instance.save.servalExLevel >= 5;
        if (targetTrans) {
            ...
            if (groundedFlag) {
                if (attackProcess == 0 || (attackProcess == 1 || attackTypeSave != 0)) {
                    if (sqrDist >= 8f * 8f || (sqrDist >= 5 * 5 && Random.Range(0, 100) < 75))
                    {
                        ...
                        if (CharacterManager.Instance.IsPlayerWaveAttacking)
                        {
                            attackIndex = 4;
                        }
                        else
                        {
                            ...
                            if (randTemp != 0 || attackTypeSave == waveAttackIndex)
                            {
                                attackIndex = 2;
                            }
                            else
                    ...
                    if (attackIndex == 1 && !JudgeStamina(GetCost(CostType.Jump) + GetCost(CostType.Run) + GetCost(CostType.Attack))) { // JumpAttack
                        attackIndex = 0;
                    } else if (attackIndex == 2 && (!JudgeStamina(GetPileCost()) || sqrDist <= 2f * 2f)) { // PileAttack
                        attackIndex = 0;
                    } else if (attackIndex == 3 && !JudgeStamina(GetSpinCost())) { // SpinAttack
                        attackIndex = 0;
                    } else if (attackIndex == 4 && (!JudgeStamina(GetWaveCost()) || (sqrDist < MyMath.Square(1.5f + waveAttackedTimeRemain * 0.5f) && Random.Range(0, 100) < 75))) { // WaveAttack
                        attackIndex = 0;
                    }

                    if (attackIndex == 2 && !(CharacterManager.Instance.GetEquipEffect(CharacterManager.EquipEffect.Pile) != 0 && GameManager.Instance.save.servalExLevel >= 3))
                    {
                        attackIndex = 0;
                    }
                    if (attackIndex == 3 && !(CharacterManager.Instance.GetEquipEffect(CharacterManager.EquipEffect.Spin) != 0 && GameManager.Instance.save.servalExLevel >= 3))
                    {
                        attackIndex = 0;
                    }
                    if (attackIndex == 4 && !(CharacterManager.Instance.GetEquipEffect(CharacterManager.EquipEffect.Wave) != 0 && GameManager.Instance.save.servalExLevel >= 4))
                    {
                        attackIndex = 0;
                    }
                ...
                if (attackIndex == 1) {
                    ...
                    attackBiasValue = 0;
                    if (AttackBase(pileAttackIndex, 1.5f, 1.5f, GetPileCost(), 15f / 30f / spRate, 15f / 30f / spRate, 0f, spRate, true, 20f)) {
                        PerfectLockon();
                        S_ParticlePlay(0);
                        attackBiasValue = 0;
                    ...
                    if (AttackBase(spinAttackIndex, 1f, 1f, GetSpinCost(), spinInterval, spinInterval, 0.5f, spinSp)) {
                        SpecialStep(0.4f, 0.25f, 4f, 0f, 0f, true, true, EasingType.SineInOut, true);
                        attackProcess = 0;
                        isSkillAttacking = true;
                        SuperarmorStart();
                        EmitEffect(isPlasma ? effectSpinSuper : effectSpin);
                    }
                } else if (attackIndex == 4) {
                    spRate = isSuperman ? 4f / 3f : 10f / 9f;
                    if (AttackBase(waveAttackIndex, 1f, 1f, GetWaveCost(), 24f / 30f / spRate, 24f / 30f / spRate, 0, spRate, true, 24f)) {
                        attackProcess = 0;
                        isSkillAttacking = true;
                    ...
                    if (AttackBase(1, 1.15f, 1.2f, GetCost(CostType.Attack), 10f / 30f / spRate, 10f / 30f / spRate, 1f, spRate)) {
                        ...
                        if (CharacterManager.Instance.GetEquipEffect(CharacterManager.EquipEffect.Combo) != 0 && GameManager.Instance.save.servalExLevel >= 2)
                        {
                            attackProcess = 2;
                        }
                        else
                ...
                if (JudgeStamina(GetBoltCost()) && CharacterManager.Instance.GetEquipEffect(CharacterManager.EquipEffect.Bolt) != 0 && GameManager.Instance.save.servalExLevel >= 5) {
                    if (AttackBase(boltAttackIndex, 1f, 1f, GetBoltCost(), 5f, 15f / 30f / spRate, 0f, 1, true, 20f)) {
                        S_ParticlePlay(0);
                        attackProcess = 0;
                        attackBiasValue = 0;
                        isSkillAttacking = true;
                    ...
                    if (AttackBase(jumpAttackIndex, 1f, 2f, GetCost(CostType.Attack), 5f, 15f / 30f / spRate, 0f, 1, true, 20f)) {
                        S_ParticlePlay(0);
                        attackProcess = 0;
                        attackBiasValue = 0;
                        isSkillAttacking = false;
    ...
    protected override void Jump(float power)
    {
        if (JudgeStamina(GetCost(CostType.Jump)))
        {
            ...
            if (JudgeStamina(GetScrewCost())
                && (isSuperman || (targetTrans && GetTargetDistance(true, true, true) <= 1.5f * 1.5f) || Random.Range(0, 100) < 40)
                && (CharacterManager.Instance.GetEquipEffect(CharacterManager.EquipEffect.Screw) != 0 && GameManager.Instance.save.servalExLevel >= 4))
            {
                nowST -= GetScrewCost();
    ...
    public override void AnimationStopTiming() {
        if (state == State.Attack && !isAnimEnd && !isAnimStopped) {
            ...
            switch (attackType) {
                case jumpAttackIndex:
                    anim.Play(PlayerController_Serval.animStateHashJump, 0, PlayerController_Serval.animStopNormalizedTimeJump);
                    break;
                case boltAttackIndex:
                    anim.Play(PlayerController_Serval.animStateHashBolt, 0, PlayerController_Serval.animStopNormalizedTimeBolt);
                    break;
                case pileAttackIndex:
                    anim.Play(PlayerController_Serval.animStateHashPile, 0, PlayerController_Serval.animStopNormalizedTimePile);
                    break;
            }
        }
    ...
    protected override void Update_Process_Jump() {
        ...
        if (targetTrans && (command == Command.Default || command == Command.Free)) {
            ...
            if (JudgeStamina(GetCost(CostType.Attack))) {
                ...
                move += GetTargetVector(true, true, true) * GetMaxSpeed(false, false, false, true) * Mathf.Clamp01(0.5f + stateTime * 2f);
            }
        }
    }
        ...
        switch (attackType) {
            ...
            case 5:
                ...
                if (!isAnimEnd) {
                    ...
                    move += trans.TransformDirection(vecForward) * speedTemp;
                } else {
    ...
    public void SetServalLevel(int newLevel)
    {
        int servalLevelSave = GameManager.Instance.save.servalExLevel;
        GameManager.Instance.save.servalExLevel = Mathf.Clamp(newLevel, 1, GameManager.servalExLevelMax);
        float multiplier = Mathf.Clamp01(0.25f + GameManager.Instance.save.servalExLevel * 0.15f);
        maxST = maxSTBase * multiplier;
        attackPower = attackPowerBase * multiplier;
        knockPower = knockPowerBase * multiplier;
        dodgePower = dodgePowerBase * multiplier;
        nowST = GetMaxST();
        if (dodgeRemain > dodgePower)
        {
            dodgeRemain = dodgePower;
        }
        if (GameManager.Instance.save.servalExLevel != servalLevelSave)
        {
            MessageUI.Instance.SetMessageReplace(TextManager.Get("ITEM_NAME_1" + friendsId.ToString("00")) + " " + GameManager.Instance.GetServalExLevelText(), "SET_SERVAL_LEVEL", 4, MessageUI.panelType_Information, GameManager.Instance.save.servalExLevel >= GameManager.servalExLevelMax ? MessageUI.slotType_Friends : MessageUI.slotType_Normal);
        }
    }

    public void IncrementServalLevel()
    {
        SetServalLevel(Mathf.Clamp(GameManager.Instance.save.servalExLevel + 1, 1, GameManager.servalExLevelMax));
        AddNowHP(Mathf.RoundToInt(Mathf.Max(40, GetMaxHP() * 7 / 100)), GetCenterPosition(), true, CharacterManager.colorTypeHeal);
        CharacterManager.Instance.EmitEffect((int)EffectDatabase.id.itemJaparibun01, friendsId, true, true, false);
    }

    public override void HitAttackAdditiveProcess()
    {
        if (GameManager.Instance.save.servalExLevel >= 5)
        {
