...
public class Friends_AnotherServal : FriendsBase {
    ...
    protected const int superSpinAttackIndex = 9;
    ...
    protected override void Update_StatusJudge() {
        ...
        SetMultiHitIntervalForSpin();
    }

    void SetMultiHitIntervalForSpin()
    {
        float interval = 0f;
        float defaultIntervalTemp = 0f;
        int maxHitCount = 0;
        if (state == State.Attack && attackType == superSpinAttackIndex)
        {
            defaultIntervalTemp = 0.5f;
            interval = 0.5f;
            maxHitCount = 3;
        }
        if (isMultiBuff)
        {
            interval = multiHitBuffInterval;
            maxHitCount = 0;
        }
        for (int i = 0; i < defaultMultiHitInterval.Length; i++)
        {
            if (i == rightDet || i == leftDet)
            {
                defaultMultiHitInterval[i] = defaultIntervalTemp;
            }
        }
        for (int i = 0; i < attackDetection.Length; i++)
        {
            if (i == rightDet || i == leftDet)
            {
                attackDetection[i].multiHitInterval = interval;
                attackDetection[i].multiHitMaxCount = maxHitCount;
            }
        }
    }
    ...
    float GetSpinCost()
    {
        return GetCost(CostType.Skill) * 1.5625f;
    }

    float GetSuperSpinCost()
    {
        return GetCost(CostType.Skill) * 2.5f;
    }

    float GetWaveCost()
    {
        return GetCost(CostType.Skill) * 1.25f;
    }
        ...
        if (targetTrans) {
            ...
            if (groundedFlag) {
                ...
                if (attackIndex == 1) {
                    ...
                    bool superSpinEnabled = (isSuperman && JudgeStamina(GetSuperSpinCost()));
                    if (AttackBase(superSpinEnabled ? superSpinAttackIndex : spinAttackIndex, 1f, 1f, superSpinEnabled ? GetSuperSpinCost() : GetSpinCost(), spinInterval, spinInterval, 0.5f, spinSp)) {
                        SpecialStep(0.4f, 0.25f, 4f, 0f, 0f, true, true, EasingType.SineInOut, true);
                        attackProcess = 0;
                        isSkillAttacking = true;
                        spinMoving = false;
                        SuperarmorStart();
                        if (attackType != superSpinAttackIndex) {
                    ...
                    if (AttackBase(waveAttackIndex, 1f, 1f, GetWaveCost(), 30f / 30f / spRate, 30f / 30f / spRate, 0, spRate, true, 24f)) {
                        attackProcess = 0;
                        isSkillAttacking = true;
    ...
    public void SacrificeOnPlayerResurrection() {
        ...
        if (CharacterManager.Instance.playerIndex == GameManager.playerIndex_HyperServal) {
    ...
    protected override void AttackContinuous() {
        ...
        switch (attackType) {
            ...
            case pileAttackIndex:
                ...
                break;
            case spinAttackIndex:
            case superSpinAttackIndex:
                if (spinMoving) {
                    ...
                    if (targetTrans && GetTargetDistance(true, true, true) >= 0.4f) {
                        ...
                        move += GetTargetVector(true, true) * GetMaxSpeed(false, false, false, true) * 0.9f * Mathf.Clamp01(spinMovingTime * 3f);
                    }
                }
                break;
        }
