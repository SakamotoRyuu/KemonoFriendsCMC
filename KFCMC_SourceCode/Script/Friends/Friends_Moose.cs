...
public class Friends_Moose : FriendsBase {
    ...
    void MoveAttack3() {
        ...
        SpecialStep(-1.5f, 12f / 30f / spRate, 8f, 4f, 5f, true, false, EasingType.SineOut);
    }
    ...
    protected override void Attack() {
        ...
        if (attackProcess == 0 && Random.Range(0f, 6f + tackledTimeRemain) < 3f && JudgeStamina(GetCost(CostType.Skill))) {
            ...
            switch (attackProcess) {
                case 0:
                    if (AttackBase(0, 1f, 1f, GetCost(CostType.Attack), 42f / 30f / spBias / spRate, 42f / 30f / spBias / spRate, 0.75f, spBias * spRate)) {
                        S_ParticlePlay(0);
                        MoveStart(0);
                    }
                    break;
                case 1:
                    if (AttackBase(1, 1f, 1f, GetCost(CostType.Attack), 42f / 30f / spBias / spRate, 42f / 30f / spBias / spRate, 0.75f, spBias * spRate)) {
                        S_ParticlePlay(0);
                    }
                    break;
                case 2:
                    if (AttackBase(2, 1.15f, 1.2f, GetCost(CostType.Attack) * (48f / 42f), 48f / 30f / spBias / spRate, 48f / 30f / spBias / spRate, 0.75f, spBias * spRate)) {
                        S_ParticlePlay(0);
                    }
                    break;
                case 3:
                    if (AttackBase(GetTargetHeight(true) >= 1.2f ? 5 : 3, 1.3f, 2f, GetCost(CostType.Attack) * (57f / 42f), 57f / 30f / spBias / spRate, 70f / 30f / spBias / spRate, 0.75f, spBias * spRate)) {
                        S_ParticlePlay(0);
                        tackledTimeRemain = 0f;
                    }
                    break;
