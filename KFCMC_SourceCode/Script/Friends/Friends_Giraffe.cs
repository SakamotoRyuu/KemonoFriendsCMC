...
public class Friends_Giraffe : FriendsBase {
    ...
    protected override void Awake() {
        ...
        if (animatorForBattle) {
            ...
            moveCost.skill = 40f * staminaCostRate;
    ...
    protected override void Attack() {
        ...
        if (attackProcess == 0 && spinAttackedTimeRemain <= 0f && JudgeStamina(GetCost(CostType.Skill)) && (Random.Range(0, 100) < 75 || searchArea[0].IsBesieged(CharacterManager.Instance.GetBuff(CharacterManager.BuffType.Long) ? 10f : 3.5f))) {
            if (AttackBase(heightDist >= 1.4f ? 5 : 4, 1.1f, 1.25f, GetCost(CostType.Skill), 28f / 30f / 0.7f / spRate, 28f / 30f / 0.7f / spRate, 1f, 0.7f)) {
                S_ParticlePlay(0);
                S_ParticlePlay(1);
                SuperarmorStart();
                MoveAttack2();
            ...
            if (attackProcess == 2 && JudgeStamina(GetCost(CostType.Attack) * (45f / 24f))) {
                if (AttackBase(3, 1.15f, 1.15f, GetCost(CostType.Attack) * (45f / 24f) , 45f / 30f / spRate, 45f / 30f / spRate, 1f, spRate)) {
                    S_ParticlePlay(2);
                    S_ParticlePlay(3);
                    SuperarmorStart();
                    MoveAttack1();
                ...
                if (AttackBase(2, 1.3f, 1.3f, GetCost(CostType.Attack) * (33f / 24f), 33f / 30f / spRate, 33f / 30f / spRate, 1f, spRate)) {
                    S_ParticlePlay(2);
                    MoveAttack1();
                    attackProcess = 2;
                }
                ...
                if (attackProcess == 0) {
                    if (AttackBase(0, 1f, 1f, GetCost(CostType.Attack), 24f / 30f / spRate, 24f / 30f / spRate, 1f, spRate)) {
                        S_ParticlePlay(2);
                        MoveAttack1();
                        attackProcess = 1;
                    }
                } else {
                    if (AttackBase(1, 1.25f, 1.25f, GetCost(CostType.Attack) * (30f / 24f), 30f / 30f / spRate, 30f / 30f / spRate, 1f, spRate)) {
                        S_ParticlePlay(2);
                        MoveAttack1();
                        attackProcess = 2;
                    }
