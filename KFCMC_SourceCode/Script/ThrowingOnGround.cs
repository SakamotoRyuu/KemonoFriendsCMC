...
public class ThrowingOnGround : ThrowingRandom {
    ...
    public override void ThrowStart(int index) {
        ...
        else if (insteadPivot)
        {
            throwSettings[index].from.transform.position = insteadPivot.position;
            base.ThrowStart(index);
            if (applyGroundNormal && throwSettings[index].instance)
            {
                throwSettings[index].from.transform.rotation = insteadPivot.rotation;
            }
        }
        else { 
            ThrowCancelAll();
