...
public class Enemy_Euglena : EnemyBase
{
    ...
    protected override void Update_StatusJudge()
    {
        ...
        if (targetSave != target)
        {
            targetSave = target;
            if (target)
            {
                targetCharacterBase = target.GetComponentInParent<CharacterBase>();
            }
            else
            {
                targetCharacterBase = null;
            }
        }
    }
    ...
    protected override void Attack()
    {
        ...
        if (targetTrans)
        {
            ...
            if (attackRand == 0 || attackRand == 1)
            {
                ...
                if (level >= 5 && targetCharacterBase && targetCharacterBase.GetPredictedPositionY(0.5f) >= trans.position.y + 1.2f)
                {
                    // Up
                    if (attackRand != 0)
                    {
                        AttackBase(7, 1, 0.8f, 0, 50f / 30f, 50f / 30f + GetAttackInterval(1f, -1), 0.5f);
                    }
                    else
                    {
                        AttackBase(6, 1, 0.8f, 0, 40f / 30f, 40f / 30f + GetAttackInterval(1f), 0.5f);
                    }
                }
                else if (Vector3.Cross(trans.TransformDirection(vecForward), targetTrans.position - trans.position).y > 0)
                {
                    //Right
                    if (attackRand != 0)
                    {
