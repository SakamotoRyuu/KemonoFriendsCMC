...
public class Enemy_DarkServal : EnemyBaseBoss {
    ...
    public SphereCollider screwAttackCollider;
    ...
    private const int effectOpenWing = 15;
    private float counterDodgeTimeRemain;
    private const float counterDodgeTimeMin = 3f;
    private const float counterDodgeTimeMax = 4f;
    ...
    protected override void Awake() {
        ...
        if (isQuestionServal)
        {
            sandstarRawKnockEnduranceLight = 4999;
        }
        ...
        if (isQuestionServal)
        {
            attackWaitingLockonRotSpeed = 8f;
            attackLockonDefaultSpeed = 20f;
        }
    }
    ...
    public override int GetStealItemID() {
        if (stealedCount < stealedMax) {
            stealedCount++;
            return (stealedCount == stealedMax ? (int)ItemDatabase.ItemID.CrystalYellow : (int)ItemDatabase.ItemID.CrystalBlack);
        }
    ...
    protected override void Update_StatusJudge() {
        ...
        if (isQuestionServal)
        {
            ...
            if (!battleStarted && target && targetFound && disableControlTimeRemain <= 0f)
            {
                BattleStart();
            }
        }
        ...
        CheckDynamicBone();
        // QuestionServal
        if (state == State.Damage && counterDodgeTimeRemain > 0f)
        {
            counterDodgeTimeRemain -= deltaTimeMove * CharacterManager.Instance.riskyIncrease * (weakProgress == 2 ? 2f : 1f);
        }
    }
    ...
    void LateUpdate()
    {
        ...
        if (isQuestionServal)
        {
            if (battleStarted && hyperJumpEffect.enabled && !despairActivated)
            {
                if (state != State.Jump && groundedFlag)
                {
                    SetAnimationWing(0);
                }
                else if (attackDetection.Length > screwDet && attackDetection[screwDet] && state == State.Jump && attackDetection[screwDet].attackEnabled)
                {
                    SetAnimationWing(1);
                }
            }
    ...
    protected override void KnockLightProcess() {
        ...
        if (!isCoreShowed && state != State.Dead && !isReaper) {
            ...
            if (isQuestionServal)
            {
                counterDodgeTimeRemain = Random.Range(counterDodgeTimeMin, counterDodgeTimeMax);
            }
        }
        if (cannotDodgeTimeRemain < 0.8f) {
            cannotDodgeTimeRemain = 0.8f;
        }
        if (isQuestionServal && state != State.Dead && counterDodgeTimeRemain <= 0f)
        {
            CounterDodge(Random.Range(-1, 2), true);
            counterDodgeTimeRemain = Random.Range(counterDodgeTimeMin, counterDodgeTimeMax);
        }
    }
    ...
    public override void EmitEffectString(string type) {
        switch (type) {
            ...
            case "DeadSave":
                if (!isDeadEffectUsed)
                {
                    isDeadEffectUsed = true;
                    DeadEvent();
                    EmitEffect(effectDeadSave);
                }
                break;
            case "OpenWing":
                EmitEffect(effectOpenWing);
    ...
    protected override void Start_Process_Dead()
    {
        ...
        EmitEffectString("DeadSave");
    }

    float GetAttackIntervalSpecial() {
        float interval;
        if (weakProgress >= 2 || (enemyCanvasLoaded && enemyBalloonElements.IbisSong.activeSelf)){
    ...
    public override void MakeAngry(GameObject decoyObject, CharacterBase attacker = null, bool isAttracterPlayer = false) {
    ...
    void ScrewStart() {
        screwAttackCollider.radius = 0.01f;
        AttackStart(screwDet);
    }

    void ScrewStartSecond()
    {
        screwAttackCollider.radius = 0.6f;
    }
    ...
    {
        if (isQuestionServal && attackType == 6)
        {
            // ハテナサーバルはスピンアタック時に爪の判定を出さない
        }
        else
        {
        ...
        if (targetTrans) {
            ...
            if (groundedFlag) {
                ...
                if (attackIndex == 1) {
                    ...
                    attackBiasValue = 0;
                    float spinSp = 3f / 4f;
                    float spinInterval = (exLevel <= 2 || isQuestionServal ? 28f : 58f) / 30f / spinSp;
                    if (AttackBase(6, 1.1f, 1.5f, 0, spinInterval, spinInterval + intervalPlus, 0.5f, spinSp)) {
                        ...
                        } else if (exLevel == 2 || isQuestionServal) {
    ...
    protected override void Update_Process_Jump() {
        ...
        if (targetTrans) {
            ...
            if (GetTargetDistance(true, true, false) > 0.1f * 0.1f) {
                ...
                move += GetTargetVector(true, true) * GetAirMaxSpeed() * (stateTime < 0.5f ? stateTime * 2f : 1f);
            }
        }
    }
    ...
    public override void AnimationStopTiming() {
        if (state == State.Attack && !isAnimEnd && !isAnimStopped) {
            ...
            switch (attackType) {
                case 4:
                    anim.Play(PlayerController_Serval.animStateHashJump, 0, 0.3725f);
                    break;
                case 8:
                    anim.Play(PlayerController_Serval.animStateHashBolt, 0, PlayerController_Serval.animStopNormalizedTimeBolt);
                    break;
                case 5:
                    anim.Play(PlayerController_Serval.animStateHashPile, 0, 0.2444444f);
                    break;
            }
        }
        ...
        switch (attackType) {
            ...
            case 5:
                ...
                if (!isAnimEnd) {
                    ...
                    move += trans.TransformDirection(vecForward) * pileMaxSpeed * (stateTime < 0.3f ? stateTime / 0.3f : 1);
                } else {
                    agent.velocity = vecZero;
                }
                if (!isAnimStopped) {
                    lockonRotSpeed = (isQuestionServal ? 20f : 10f + (exLevel - 1));
                } else if (!isAnimEnd) {
                    lockonRotSpeed = (isQuestionServal ? 20f : 10f + (exLevel - 1)) - Mathf.Max(0f, nowSpeed / pileMaxSpeed * 8f);
                } else {
                    lockonRotSpeed = 0f;
                }
            ...
            case 6:
                if (spinMoving) {
                    ...
                    move += trans.TransformDirection(vecForward) * GetMaxSpeed(false, false, false, true) * 0.9f * Mathf.Clamp01(spinMovingTime * 3f);
                }
                break;
        }
    }
    ...
    {
        // 石常時露出状態の場合は0を返す
        if (!isReaper && isCoreShowed && state != State.Dead && weakProgress == 2)
        {
            return 0;
        }
        else
