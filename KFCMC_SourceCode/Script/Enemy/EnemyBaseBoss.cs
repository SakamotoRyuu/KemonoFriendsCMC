...
public class EnemyBaseBoss : EnemyBase {
    ...
    public enum CoreShowType { 
        Always, // Alligator clips
        KnockLight, // Dark Serval
        KnockHeavyStop, // B-2
        KnockHeavyMove, // Akula
        Other // BigDog, BigDogInside
    }
    ...
    public CoreShowType coreShowType;
    ...
    protected int coreHideBorderStartHP;
    ...
    protected bool forceRecoveryRateZero;
    protected bool forceChimeraGauge;

    protected const float chimeraHealReserveTimeMax = 10;
    ...
    protected override void LoadEnemyCanvas() {
        ...
        // ボスセルリアンは通常のゲージを無効にする
        enemyCanvasElements.BackNormal.SetActive(false);
        enemyCanvasElements.BackWRFill.SetActive(false);
        enemyCanvasElements.HpDiff.SetActive(false);
        enemyCanvasElements.HpFill.SetActive(false);
        enemyCanvasElements.KnockRecovery.SetActive(false);
        enemyCanvasElements.KnockDiff.SetActive(false);
        enemyCanvasElements.KnockFill.SetActive(false);
        enemyCanvasElements.WrFill.SetActive(false);
    }
    ...
    protected bool CanChimeraHeal()
    {
        return nowHP > 0 && chimeraHeal.healRate > 0f && GameManager.Instance.save.difficulty >= chimeraHeal.conditionDifficulty && (sandstarRawEnabled || !chimeraHeal.isSandstarRawOnly);
    }

    protected bool CanChimeraHealForBuffCount(int buffCount)
    {
        return chimeraHeal.conditionBuffCount <= 0 || buffCount >= chimeraHeal.conditionBuffCount || (CharacterManager.Instance.costSumSave > CharacterManager.riskyCost && buffCount >= chimeraHeal.conditionBuffCount - 1);
    }
    ...
    protected override void Update_StatusJudge() {
        ...
        if (CanChimeraHeal()) {
            ...
            if (Time.timeScale > 0f && chimeraHealReserveTimeRemain <= 0f && nowHP <= Mathf.RoundToInt(GetMaxHP() * (1f - chimeraHeal.healRate))) {
                ...
                if (CanChimeraHealForBuffCount(buffCount)) {
                    if (chimeraHeal.effectPrefab && centerPivot) {
                        Instantiate(chimeraHeal.effectPrefab, centerPivot);
                    }
                    chimeraHealReserveTimeRemain = chimeraHealReserveTimeMax;
    ...
    public override int GetStealItemID() {
        ...
        return (int)ItemDatabase.ItemID.HealStar;
    }
    ...
    public override float GetKnockGaugeNow()
    {
        if (coreShowType == CoreShowType.KnockLight && state != State.Dead)
        {
            if (isCoreShowed)
            {
                return 0;
            ...
            {
                return knockRemainLight;
            }
        }
        else if (coreShowType == CoreShowType.KnockHeavyMove && state != State.Dead)
        {
            if (isCoreShowed)
            {
                return 0;
        ...
        return base.GetKnockGaugeNow();
    }

    public override float GetKnockGaugeMax()
    {
        if (coreShowType == CoreShowType.KnockLight)
        {
            return knockEnduranceLight;
        }
        return base.GetKnockGaugeMax();
    }

    public override bool KnockRecoveryGaugeEnabled()
    {
        if (coreShowType == CoreShowType.KnockLight || coreShowType == CoreShowType.KnockHeavyMove)
        {
            return (isCoreShowed || (state == State.Damage && isDamageHeavy)) && state != State.Dead;
        }
        return base.KnockRecoveryGaugeEnabled();
    }

    public override float GetKnockRecoveryGaugeRate()
    {
        if (forceRecoveryRateZero)
        {
            return 0;
        }
        if (state != State.Dead)
        {
            bool calcForCoreShow = false;
            switch (coreShowType)
            {
                case CoreShowType.Always:
                    break;
                case CoreShowType.KnockLight:
                case CoreShowType.KnockHeavyMove:
                    calcForCoreShow = (isCoreShowed || (state == State.Damage && isDamageHeavy)) && state != State.Dead;
                    break;
                case CoreShowType.KnockHeavyStop:
                    calcForCoreShow = state == State.Damage && isDamageHeavy;
                    break;
            }
            if (calcForCoreShow)
            {
                return Mathf.Clamp01(Mathf.Max(
                    coreShowHP - coreHideBorderStartHP > 0 ? 1 - (float)(nowHP - GetCoreHideBorder()) / (coreShowHP - coreHideBorderStartHP) : 0,
                    coreTimeMax > 0 ? 1 - coreTimeRemain / coreTimeMax : 0
                    ));
            }
        }
        return base.GetKnockRecoveryGaugeRate();
    }

    public override bool ChimeraGaugeEnabled()
    {
        if (forceChimeraGauge)
        {
            return true;
        }
        else if (CanChimeraHeal())
        {
            int buffCount = CharacterManager.Instance.GetBuffCount(true);
            if (CanChimeraHealForBuffCount(buffCount))
            {
    ...
    public override float GetChimeraGaugeRate()
    {
        if (CanChimeraHeal())
        {
            int buffCount = CharacterManager.Instance.GetBuffCount(true);
            if (CanChimeraHealForBuffCount(buffCount))
            {
                return Mathf.Clamp01(1 - chimeraHealReserveTimeRemain / chimeraHealReserveTimeMax);
            }
        }
        return 0;
    }
