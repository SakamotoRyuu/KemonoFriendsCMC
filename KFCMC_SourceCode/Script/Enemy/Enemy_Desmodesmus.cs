...
public class Enemy_Desmodesmus : EnemyBase
{
    ...
    float arriveIntervalTimer;
    float searchAreaEnablizeTimer;

    static readonly float[] lockonSpeedArray = new float[] { 0.4f, 0.4f, 0.6f, 0.9f, 1.3f, 2f };
    static readonly float[] lineMaxDistArray = new float[] { 15f, 15f, 20f, 20f, 20f, 20f };
    ...
    protected override void Update_TimeCount()
    {
        ...
        if (arriveIntervalTimer > 0)
        {
            arriveIntervalTimer -= deltaTimeCache;
        }
    }
    ...
    private void LateUpdate()
    {
        for (int i = 0; i < linePivot.Length; i++)
        {
            if (GetCanControl() && attackedTimeRemain <= 0 && i < lineMax)
            {
                ...
                if (Physics.Raycast(ray, out raycastHit, lineMaxDistArray[Mathf.Clamp(level, 0, lineMaxDistArray.Length - 1)], lineLayerMask, QueryTriggerInteraction.Ignore))
                {
                ...
                {
                    lineRenderer[i].SetPosition(1, vecForward * (lineMaxDistArray[Mathf.Clamp(level, 0, lineMaxDistArray.Length - 1)] * lineLengthBias));
                }
            }
            else
            {
    ...
    {
        base.Attraction(decoy, type, lockForce, targetFixingTime, isAttracterPlayer);
        SetOffensive();
    }
    ...
    protected override void AttackContinuous()
    {
        ...
        if (movingFlag && targetTrans)
        {
            ...
            move += 21f * trans.TransformDirection(vecForward);
        }
    }
