...
public class Enemy_Halocynthia : EnemyBase
{
    ...
    public AttackDetectionSick sickAttackDetection;
    ...
    protected override void SetLevelModifier()
    {
        ...
        attackSave = -1;
        if (sickAttackDetection)
        {
            sickAttackDetection.additionalSicks[0].probability = level >= 5 ? 100 : 0;
        }
    }
    ...
    void SetBomb(int exceptIndex)
    {
        for (int i = 0; i < bombPivot.Length; i++)
        {
            ...
            else if (bombNotExistingTime[i] >= 0f && bombInstance[i] == null)
            {
                ...
                bombNotExistingTime[i] = Random.Range(-6f, -10f) * (level >= 5 ? 0.5f : 1f);
            }
        }
    }
    ...
    protected override void Update_StatusJudge()
    {
        ...
        float bombEmitTime = level >= 5 ? 4.5f : 9f;
        for (int i = 0; i < bombInstance.Length && i < bombNotExistingTime.Length; i++)
        {
            if (bombNotExistingTime[i] < bombEmitTime && bombInstance[i] == null)
            {
                bombNotExistingTime[i] += deltaTimeCache;
                if (level >= 3 && i != nowPoint && bombNotExistingTime[i] >= bombEmitTime)
                {
                    bombInstance[i] = Instantiate(bombPrefab, bombPivot[i]);
