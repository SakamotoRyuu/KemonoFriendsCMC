...
public class Enemy_ImperatrixMundi : EnemyBaseBoss
{
    ...
    private int shieldEnemyMaxHPSum;
    ...
    private const float shieldRadius = 15f;
    ...
    private void CoreHide() {
        ...
        int shieldMaxTemp = 3;
        if (CharacterManager.Instance.playerIndex == GameManager.playerIndex_HyperServal && !isForAmusement) {
            if (sandstarRawEnabled) {
        ...
        for (int i = 0; i < shieldEnemyInstance.Length; i++) {
            if (shieldEnemyInstance[i]) {
                ...
                if (eBase) {
                    eBase.ResetHP();
                    shieldEnemyMaxHPSum += eBase.GetMaxHP();
                }
            } else {
                if (i < shieldMaxTemp) {
                    ...
                    shieldEnemyMaxHPSum += shieldEnemyBase[i].GetMaxHP();
                }
            }
        }
    }
    ...
    private void SetSandstarRawForChild() {
        shieldEnemyMaxHPSum = 0;
        for (int i = 0; i < shieldEnemyBase.Length; i++) {
            if (shieldEnemyBase[i]) {
                shieldEnemyBase[i].SetSandstarRaw();
                shieldEnemyMaxHPSum += shieldEnemyBase[i].GetMaxHP();
            }
        }
    ...
    private bool AreShieldEnemiesDefeated()
    {
        bool allNull = true;
        for (int i = 0; i < shieldEnemyInstance.Length; i++)
        {
    ...
    protected override void Update_StatusJudge() {
        ...
        if (state == State.Attack && laserLookatEnabled && enemyBalloonElements.PaperPlane.activeSelf == false && enemyBalloonElements.MargayVoice.activeSelf == false) {
            retargetingConditionTime = 0.001f;
    ...
    private void SetSunIndependent() {
        ...
        sunIndependentBase.SetParentEnemy(this, (20000 + Mathf.Clamp(GameManager.Instance.save.difficulty - 1, 0, 3) * 3000 + (sandstarRawEnabled && !isForAmusement ? 3000 : 0)) * (CharacterManager.Instance.playerIndex == GameManager.playerIndex_HyperServal ? 1f : 0.25f));
    ...
    {
        if (sunIndependentBase)
        {
            return sunIndependentBase.GetKnockGaugeNow();
        }
        else if (!isCoreShowed)
        {
            if (shieldEnemyMaxHPSum > 0)
            {
                int nowHPSum = 0;
                for (int i = 0; i < shieldEnemyBase.Length; i++)
                {
                    if (shieldEnemyBase[i])
                    {
                        nowHPSum += shieldEnemyBase[i].GetNowHP();
                    }
                }
                return nowHPSum;
            }
    ...
    public override float GetKnockGaugeMax()
    {
        if (sunIndependentBase)
        {
            return sunIndependentBase.GetKnockGaugeMax();
        }
        else if (!isCoreShowed)
        {
            if (shieldEnemyMaxHPSum > 0)
            {
                return shieldEnemyMaxHPSum;
            }
            else
            {
                return 1;
