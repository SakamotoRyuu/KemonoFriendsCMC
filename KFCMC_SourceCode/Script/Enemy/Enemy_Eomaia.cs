...
public class Enemy_Eomaia : EnemyBase
{
    ...
    public float bornIntervalTimeMax = 40;
    public int bornCountMax = 40;
    ...
    int attackSave = -1;
    bool isAttackForBorn;
    float bornIntervalTimeRemain;
    ...
    {
        bornIntervalTimeRemain = bornIntervalTimeMax;
    }
    ...
    protected override void Update_StatusJudge()
    {
        ...
        {
            isAttackForBorn = false;
        }
        // 分裂行動
        if (level >= 5 && GetCanControl())
        {
            bornIntervalTimeRemain -= deltaTimeCache;
            if (bornIntervalTimeRemain < 0 && target == null)
            {
                if (StageManager.Instance.dungeonController.EnemyCount() < bornCountMax)
                {
                    bornIntervalTimeRemain = bornIntervalTimeMax;
                    isAttackForBorn = true;
                    SetState(State.Attack);
                }
                else
                {
                    bornIntervalTimeRemain = bornIntervalTimeMax * 0.5f;
                }
            }
        }
    }
    ...
    */
    ...
    protected override void Attack()
    {
        ...
        {
            agent.radius = 0.1f;
            AttackBase(3, 0, 0, 0, 65f / 30f, 65f / 30f + GetAttackInterval(1), 0f);
            return;
        }
    ...
    protected override void AttackContinuous()
    {
        ...
        {
            ApproachTransformPivot(movePivot[movingIndex], GetMinmiSpeed() * movingSpeedMul, 0.5f * movingBrakeMul, 0.02f * movingBrakeMul, true);
        }
    }
