...
public class Enemy_BigDogInside : EnemyBaseBoss {
    ...
    private float GetSummonConditionHP(int pointerShift = 0)
    {
        return Mathf.RoundToInt(GetMaxHP() * ((float)(summonID.Length + 1 - summonPointer + pointerShift) / (summonID.Length + 2)));
    }
    ...
    public override void TakeDamage(int damage, Vector3 position, float knockAmount, Vector3 knockVector, CharacterBase attacker = null, int colorType = 0, bool penetrate = false) {
        ...
        if (nowHP > 0) {
            ...
            while (summonPointer < summonID.Length && nowHP <= GetSummonConditionHP()) {
    ...
    {
        if (summonPointer < summonID.Length)
        {
            return Mathf.Clamp(nowHP - GetSummonConditionHP(), 0, GetSummonConditionHP(1));
        }
        else
        {
            return 0;
    ...
    public override float GetKnockGaugeMax()
    {
        if (summonPointer < summonID.Length)
        {
            return GetSummonConditionHP(1) - GetSummonConditionHP();
        }
        else
        {
            return 1;
