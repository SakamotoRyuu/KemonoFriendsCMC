...
public class Enemy_Anomalocaris : EnemyBase
{
    ...
    public DamageDetection[] guardDamageDetections;
    public ParticleSystemPool.ParticleName poolParticleNameNormal;
    public ParticleSystemPool.ParticleName poolParticleNameGuard;
    public AudioSourcePool.AudioName poolAudioNameNormal;
    public AudioSourcePool.AudioName poolAudioNameGuard;
    ...
    {
        throwing.ThrowReady(throwIndexBase + 0);
    }
    ...
    void ThrowStartSpecial()
    {
        ...
        if (throwCount == 0)
        {
            throwing.ThrowStart(throwIndexBase + 0);
        }
        else if (throwCount < throwMax)
        {
            throwing.ThrowStart(throwIndexBase + 1);
        }
        throwCount++;
        if (throwCount < throwMax)
        {
            throwing.ThrowReady(throwIndexBase + 1);
        }
    }
    ...
    protected override void Update_StatusJudge()
    {
        ...
        if (throwRushTimeRemain > 0f)
        {
            ...
            if (throwRushIntervalRemain <= 0f)
            {
                ...
                if (throwCount < 12)
                {
                    throwing.ThrowStart(rushThrowIndex[throwIndexBase + throwCount % rushThrowIndex.Length]);
                    throwCount++;
                }
            }
        }
    ...
    protected override void SetLevelModifier()
    {
        ...
        if (level >= 5)
        {
            for (int i = 0; i < guardDamageDetections.Length; i++)
            {
                guardDamageDetections[i].poolParticleNames[0] = poolParticleNameGuard;
                guardDamageDetections[i].poolAudioNames[0] = poolAudioNameGuard;
                guardDamageDetections[i].defenceRate = 20;
                guardDamageDetections[i].knockedRate = 0;
                guardDamageDetections[i].colorType = 7;
            }
        }
        else
        {
            for (int i = 0; i < guardDamageDetections.Length; i++)
            {
                guardDamageDetections[i].poolParticleNames[0] = poolParticleNameNormal;
                guardDamageDetections[i].poolAudioNames[0] = poolAudioNameNormal;
                guardDamageDetections[i].defenceRate = 1;
                guardDamageDetections[i].knockedRate = 1;
                guardDamageDetections[i].colorType = 2;
            }
        }
    }
    ...
    protected override void AttackContinuous()
    {
        ...
        if (josouChaseFlag && isLockon && specialMoveDuration > 0f)
        {
