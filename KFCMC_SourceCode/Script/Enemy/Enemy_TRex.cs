...
public class Enemy_TRex : EnemyBase
{
    ...
    public RaycastToAdjustCapsuleCollider[] raycasters;
    public Transform targetChaser;
    public CheckTriggerStay backChecker;
    public Transform meteorTargetPivot;

    int coreHPSave;
    int coreIndex = -1;
    int movingIndex = -1;
    float movingSpeedMul = 1f;
    float movingSpeedOnNearMul = 1f;
    int attackSave = -1;
    ...
    const int throwMeteorSelf = 0;
    const int throwMeteorTarget = 1;
    const int throwBomb = 2;
    const int throwFirePole = 3;
    ...
    static readonly int[] atkLaserBodies = { 5, 6, 7 };
    ...
    void Meteor()
    {
        if (state == State.Attack)
        {
            ...
            throwing.ThrowStart(throwMeteorSelf);
            if (level >= 5 && targetTrans)
            {
                Vector3 selfPosition = trans.position;
                Vector3 targetPosition = targetTrans.position;
                targetPosition.y = selfPosition.y;
                Vector3 diff = targetPosition - selfPosition;
                if (diff.sqrMagnitude < 6f * 6f)
                {
                    targetPosition = selfPosition + diff.normalized * 6f;
                }
                meteorTargetPivot.position = targetPosition;
                Vector3 targetEulerAngles = targetTrans.eulerAngles;
                targetEulerAngles.x = meteorTargetPivot.eulerAngles.x;
                meteorTargetPivot.eulerAngles = targetEulerAngles;
                throwing.ThrowStart(throwMeteorTarget);
            }
        }
    }
    ...
    void LaserCancel()
    {
        ...
        for (int i = 0; i < laserMax; i++)
        {
            ...
            raycasters[i].Deactivate();
        }
        AttackEnd(atkLaserDummy);
        for (int i = 0; i < laserMax; i++)
        {
            AttackEnd(atkLaserBodies[i]);
        }
    }
    ...
    void LaserReady()
    {
        ...
        for (int i = 0; i < laserMax; i++)
        {
            ...
            raycasters[i].hitEffectEnabled = false;
            raycasters[i].Activate();
        }
    ...
    void LaserStart()
    {
        ...
        for (int i = 0; i < laserMax; i++)
        {
            ...
            raycasters[i].hitEffectEnabled = true;
            raycasters[i].Activate();
        }
        AttackEnd(atkLaserDummy);
        for (int i = 0; i < laserMax; i++)
        {
            AttackStart(atkLaserBodies[i]);
        }
    ...
    void LaserEnd()
    {
        ...
        for (int i = 0; i < laserMax; i++)
        {
            laserOptions[i].LightFlickeringBlastEnd();
            raycasters[i].Deactivate();
        }
        for (int i = 0; i < laserMax; i++)
        {
    ...
    public override void TakeDamage(int damage, Vector3 position, float knockAmount, Vector3 knockVector, CharacterBase attacker = null, int colorType = 0, bool penetrate = false)
    {
        ...
        if (changeCoreReservedTimeRemain <= 0f && colorType == damageColor_Critical && coreHPSave - nowHP >= GetMaxHP() / coreChangeDivide[Mathf.Clamp(level, 0, coreChangeDivide.Length - 1)])
        {
    ...
    {
        if (level >= 4 && throwFirePole + firePoleCount < throwing.throwSettings.Length)
        {
            throwing.ThrowStart(throwFirePole + firePoleCount);
            firePoleCount++;
        }
    }
    ...
    protected override void Attack()
    {
        ...
        {
            laserMax = level >= 5 ? laserOptions.Length : 1;
    ...
    protected override void AttackContinuous()
    {
        ...
        if (movingIndex >= 0 && movingIndex < movePivot.Length && movePivot[movingIndex] && targetTrans)
        {
            float mulTemp = movingSpeedMul;
            if (movingSpeedOnNearMul != 1f && GetTargetDistance(true, true, false) < MyMath.SqrMagnitudeIgnoreY(trans.position, movePivot[movingIndex].position))
            {
                mulTemp *= movingSpeedOnNearMul;
            }
            ApproachTransformPivot(movePivot[movingIndex], GetMinmiSpeed() * mulTemp, 0.5f);
        }
    }
