...
public class Enemy_Polistes : EnemyBase
{
    ...
    protected override void Update_StatusJudge()
    {
        ...
        if (level >= 2 && actDistNum == 0 && GetCanControl())
        {
            meleeChaseTime += deltaTimeCache;
            if (meleeChaseTime >= 8)
            {
                actDistNum = 1;
                meleeChaseTime = 0;
    ...
    void ThrowReadySpecial(int index)
    {
        if (level >= 5)
        {
            throwing.ThrowReady(index + 2);
        }
        else
        {
    ...
    void ThrowStartSpecial(int index)
    {
        if (level >= 5)
        {
            throwing.ThrowStart(index + 2);
            for (int i = 0; i < throwManyCount; i++)
            {
                throwing.ThrowStart(index + 4);
            }
        }
        else
        {
