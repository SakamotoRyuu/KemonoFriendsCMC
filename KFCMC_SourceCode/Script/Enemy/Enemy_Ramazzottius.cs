...
public class Enemy_Ramazzottius : EnemyBase {
    ...
    bool isWeaponRadiation;
    float lastAttackedTimeCount;

    const int effHeal = 5;
    const int effMolt = 6;
    const int effThrowReady = 7;
    const int ThrowIndexWeaponNormal = 0;
    const int ThrowIndexWeaponRadiation = 1;
    static readonly Vector3 moltDirection = new Vector3(0, 1, -1);
    static readonly int[] criticalNum = new int[] { 2, 2, 3, 4, 5, 6 };
    static readonly int[] damageZero = new int[] { 0, 0, 0, 6, 12, 24 };
    static readonly float[] damageRateArray = new float[] { 3.5f, 3.5f, 3.333333f, 3.166666f, 3.0f, 3.0f };
    static readonly float[] knockRestoreArray = new float[] { 1f, 1f, 1.1f, 1.21f, 1.331f, 1.331f };
    ...
    protected override void Update_StatusJudge() {
        ...
        if (level >= 3 && nowHP > 0) {
            ...
            if (healReserveTimeRemain <= 0f && GetCanControl() && nowHP <= GetMaxHP() * 4 / 5) {
                healReserveTimeRemain = level >= 5 ? 6f : 12f;
        ...
        lastAttackedTimeCount += deltaTimeCache;
        if (actDistNum == 0 && level >= 4 && trapAttackTimeRemain <= 0f && lastAttackedTimeCount >= 10f)
        {
            actDistNum = 1;
        }
    }
    ...
    protected void SetCriticalPoint() {
        ...
        for (int i = 0; i < indexes.Length; i++) {
            if (i < criticalNum[Mathf.Clamp(level, 0, criticalNum.Length - 1)]) {
    ...
    void ThrowReadyWeapon()
    {
        throwing.ThrowReady(isWeaponRadiation ? ThrowIndexWeaponRadiation : ThrowIndexWeaponNormal);
    }

    void ThrowStartWeapon()
    {
        if (state == State.Attack)
        {
            throwing.ThrowStart(isWeaponRadiation ? ThrowIndexWeaponRadiation : ThrowIndexWeaponNormal);
            trapAttackTimeRemain += 4f;
        }
        else
        {
            throwing.ThrowCancel(isWeaponRadiation ? ThrowIndexWeaponRadiation : ThrowIndexWeaponNormal);
        }
    }
