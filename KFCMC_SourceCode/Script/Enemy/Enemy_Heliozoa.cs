...
public class Enemy_Heliozoa : EnemyBaseBoss
{
    ...
    const int throwMeteorAccurateIndex = 34;
    ...
    const int effLaserCountDown = 18;
    const int effLaserCountDownNoSE = 19;
    const int effMeteorReady = 20;
    const int effMeteorStart = 21;
    const int effMeteorStartHighPriority = 22;

    const int rewardMinmiShiftNum = 4;
    static readonly int heavyKnockConditionMax = 500000;
    ...
    */
    ...
    protected override void Update_StatusJudge() {
        ...
        if (Time.timeScale > 0f) {
            if (isCoreShowed && coreCriticalTimeRemain > 0f) {
        ...
        if (targetTrans) {
            lookTargetTrans.position = MyMath.GetConsiderFallTargetPosition(targetTrans.position, meteorPivotX.position, throwing.throwSettings[throwMeteorIndex].velocity);
        ...
        switch (type) {
            ...
            case "LaserCountDown":
                if (state == State.Attack) {
                    ...
                    EmitEffect(effLaserCountDownNoSE);
                }
                break;
        }
    }
    ...
    void EmitHealEffect() {
        if (isCoreShowed && !healEffectEmitted) {
    ...
    public override void TakeDamage(int damage, Vector3 position, float knockAmount, Vector3 knockVector, CharacterBase attacker = null, int colorType = 0, bool penetrate = false) {
        if (isCoreShowed && colorType == damageColor_Critical && GetCanTakeDamage(penetrate)) {
    ...
    public override float GetKnocked() {
        return base.GetKnocked() * (!heavyKnocked && isCoreShowed && heavyKnockConditionDamage >= heavyKnockConditionMax ? 0 : 1);
    }
    ...
    void MeteorThrow() {
        if (state == State.Attack && meteorNumRemain > 0 && meteorNumMax > 0 && meteorTimeRemain <= 35f / 60f * meteorNumRemain / meteorNumMax) {
            ...
            if (!GetSick(SickType.Stop)) {
                EmitEffect(isFirstMeteor ? effMeteorStartHighPriority : effMeteorStart);
                int meteorCluster = meteorNumMax / 2;
                for (int i = 0; i < meteorCluster; i++) {
                    int index = isFirstMeteor ? throwMeteorAccurateIndex : throwMeteorIndex;
                    ...
                    throwing.throwSettings[index].angularVelocity = Random.insideUnitSphere * 4f;
        ...
        switch (attackType) {
            ...
            case attackTypeThrow:
                if (throwLookatEnabled && throwPivot && targetTrans && (targetTrans.position - throwPivot.position).sqrMagnitude > 5f * 5f) {
                    ...
                    Vector3 targetPos = MyMath.GetConsiderFallTargetPosition(targetTrans.position, fromPosition, throwing.throwSettings[0].velocity);
    ...
    public override float GetKnockRecoveryGaugeRate()
    {
        if (isCoreShowed)
        {
            return Mathf.Clamp01(Mathf.Max(
                1 - coreCriticalTimeRemain / coreCriticalTimeMax,
                (float)heavyKnockConditionDamage / heavyKnockConditionMax));
        }
