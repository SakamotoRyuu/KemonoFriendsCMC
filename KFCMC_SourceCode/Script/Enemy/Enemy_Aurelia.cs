...
public class Enemy_Aurelia : EnemyBase {
    ...
    float throwNeedleTimeRemain;
    int throwNeedleCount;
    float throwNeedleEffectInterval;
    const int ThrowNeedleIndex = 0;
    const int ThrowNeedleCountMax = 12;
    const int EffectNeedleStart = 2;
    const float ThrowNeedleTimeMax = 0.2f;
    ...
    protected override void Update_StatusJudge()
    {
        ...
        {
            if (throwNeedleTimeRemain > 0)
            {
                throwNeedleTimeRemain -= deltaTimeMove;
                throwNeedleEffectInterval -= deltaTimeCache;
                if (throwNeedleCount < ThrowNeedleCountMax && throwNeedleTimeRemain <= ThrowNeedleTimeMax - ThrowNeedleTimeMax / ThrowNeedleCountMax * throwNeedleCount)
                {
                    throwing.ThrowStart(ThrowNeedleIndex + throwNeedleCount);
                    throwNeedleCount++;
                    if (throwNeedleEffectInterval <= 0)
                    {
                        EmitEffect(EffectNeedleStart);
                        throwNeedleEffectInterval = 0.025f;
                    }
                }
            }
        }
    ...
    protected override void Attack() {
        ...
        switch (attackTemp) {
            ...
            case 1:
                ...
                break;
            case 2:
                AttackBase(2, 0.6f, 0.6f, 0, 105f / 60f, 105f / 60f + GetAttackInterval(1.5f), 0f);
                agent.radius = 0.05f;
                break;
            case 3:
    ...
    protected override void AttackContinuous() {
        ...
        if (attackType == 2 && moveContinuous != 0) {
            Continuous_Approach(Mathf.Max(10f, GetMaxSpeed(false, false, true)), 0.25f, 0.04f);
        }
    }
    ...
    void ThrowReadyNeedles()
    {
        if (level >= 5)
        {
            for (int i = 0; i < ThrowNeedleCountMax; i++)
            {
                throwing.ThrowReady(ThrowNeedleIndex + i);
            }
        }
    }

    void ThrowStartNeedles()
    {
        if (level >= 5)
        {
            LockonEnd();
            throwNeedleCount = 0;
            throwNeedleTimeRemain = ThrowNeedleTimeMax;
            throwNeedleEffectInterval = 0;
        }
    }
