...
public class Enemy_Akula : EnemyBaseBoss {
    ...
    public LaserOption[] laserOptions;
    public bool isRaw;
    public GameObject normalMissilePrefab;
    public GameObject superMissilePrefab;
    public Transform laserDistancePivot;
    public Transform[] laserRotatePivots;
    public float[] laserAngles;
    public Vector2 laserAdjustAngleDistanceRange;
    ...
    const int throwMissileIndex = 5;
    const int throwBlockIndex = 6;
    const int throwTorpedoIndex = 14;
    const int attackIndexLaser = 6;
    ...
    protected override void Update_StatusJudge() {
        ...
        if (state != State.Attack)
        {
            for (int i = 0; i < laserMax; i++)
            {
                if (laserOptions[i] != null && (laserOptions[i].isLightCharging || laserOptions[i].isBlasting))
                {
                    laserOptions[i].CancelLaser();
                }
            }
        ...
        if (followerRestRemain > 0) {
            ...
            bool followingEnabled = followerResetTimer <= 0f && (GetCanControl() || (state == State.Attack && attackType != attackIndexLaser && !throwing.GetAnyReady() && stateTime > 0.2f));
    ...
    protected override void Attack() {
        ...
        switch (attackTemp) {
            case 0://Missile
                AttackBase(0, 1f, 1f, 0, 60f / 60f, 60f / 60f + intervalPlus + childType * 0.8f);
                throwCountMax = childType * 2 + 2;
                missilePrefabSwitch = !missilePrefabSwitch;
                break;
            case 1://Block
                AttackBase(3, 1f, 1f, 0, 60f / 60f, 60f / 60f + intervalPlus + childType * 0.8f);
                throwCountMax = childType * 2 + 2;
                break;
            case 2:
                AttackBase(attackIndexLaser, 1.3f, 50f, 0, 190f / 60f, 190f / 60f + intervalPlus);
                break;
            case 3:
    ...
    void ThrowReadyMissile()
    {
        if (throwReadyCount < throwCountMax)
        {
            if (isRaw)
            {
                GameObject missilePrefab = normalMissilePrefab;
                if ((throwReadyCount + (missilePrefabSwitch ? 1 : 0)) % 2 == 1)
                {
                    missilePrefab = superMissilePrefab;
                }
                throwing.throwSettings[throwMissileIndex - throwReadyCount].prefab = missilePrefab;
            }
            throwing.ThrowReady(throwMissileIndex - throwReadyCount);
            throwReadyCount++;
        }
    }

    void ThrowStartMissile()
    {
        if (throwStartCount < throwCountMax)
        {
            throwing.ThrowStart(throwMissileIndex - throwStartCount);
            throwStartCount++;
        }
    }

    void ThrowReadyBlock()
    {
        if (throwReadyCount < throwCountMax)
        {
            if (isRaw && throwReadyCount < 2)
            {
                Throwing.ThrowSettings throwSettings = throwing.throwSettings[throwTorpedoIndex + throwReadyCount];
                Instantiate(throwSettings.prefab, throwSettings.from.transform.position, throwSettings.from.transform.rotation, throwSettings.from.transform);
            }
            throwing.ThrowReady(throwBlockIndex + throwReadyCount);
            throwReadyCount++;
        }
    }

    void ThrowStartBlock()
    {
        if (throwStartCount < throwCountMax)
        {
            if (isRaw && throwStartCount < 2)
            {
                Throwing.ThrowSettings throwSettings = throwing.throwSettings[throwTorpedoIndex + throwStartCount];
                if (throwSettings.instance != null)
                {
                    throwSettings.instance.transform.SetParent(null);
                }
            }
            ThrowStartChangeVelocity(throwBlockIndex + throwStartCount);
            throwStartCount++;
        }
    }
    ...
    void LaserReady()
    {
        if (isRaw)
        {
            ...
            if (targetTrans)
            {
                targetDistance = Mathf.Clamp(Vector3.Distance(laserDistancePivot.position, targetTrans.position), laserAdjustAngleDistanceRange.x, laserAdjustAngleDistanceRange.y);
                for (int i = 0; i < laserRotatePivots.Length; i++)
                {
                    if (laserAngles[i] != 0)
                    {
                        laserRotatePivots[i].localEulerAngles = laserAngles[i] * laserAdjustAngleDistanceRange.x / targetDistance * Vector3.up;
                    }
                }
            }
        }
        for (int i = 0; i < laserMax; i++)
        {
            laserOptions[i].LightFlickeringChargeStart();
        }
        AttackStart(0);
    }
    ...
    void LaserStart()
    {
        for (int i = 0; i < laserMax; i++)
        {
            laserOptions[i].LightFlickeringChargeEnd();
        }
        AttackEnd(0);
        for (int i = 0;i < laserMax; i++)
        {
