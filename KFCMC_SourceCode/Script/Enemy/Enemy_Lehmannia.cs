...
public class Enemy_Lehmannia : EnemyBase {
    ...
    void MoveAttack() {
        if (state == State.Attack) {
            fbStepMaxDist = 2f;
            ApproachOrSeparateBetween(4.5f, 1.5f);
        }
    }
    ...
    void EmitPoison() {
        int poisonIndex = level >= 5 ? effectPoisonSlow : effectPoison;
        ...
        if (effect[poisonIndex].pivot) {
            effect[poisonIndex].pivot.localPosition = position;
        }
        EmitEffect(poisonIndex);
        deathChecker = effect[poisonIndex].instance.GetComponent<MissingObjectToDestroy>();
