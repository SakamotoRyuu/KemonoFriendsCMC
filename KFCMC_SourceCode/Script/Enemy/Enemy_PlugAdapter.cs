...
public class Enemy_PlugAdapter : EnemyBase
{
    ...
    static readonly float[] moveDistances = new float[6] { 4f, 4f, 6f, 8f, 12f, 16f };

    void MoveAttack0()
    {
        SpecialStep(0.2f, 40f / 60f, moveDistances[Mathf.Clamp(level, 0, moveDistances.Length - 1)], 0f, 0f, true, false, EasingType.SineOut);
    }
    ...
    void StartElectricSound()
    {
        attackDetection[1].SetAttackEnabled(true);
        ParticlePlay(1);
    }
            ...
            if (attackTemp == 2)
            {
                ...
                else
                {
                    if (level >= 5)
                    {
                        ...
                        {
                            // Follow X5
                            throwReservedIndex = 3;
                        }
                        else
                        {
                            // Follow
                            throwReservedIndex = 2;
                        }
                    }
                    else
                    {
