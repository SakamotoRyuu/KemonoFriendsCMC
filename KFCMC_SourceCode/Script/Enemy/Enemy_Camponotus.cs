...
public class Enemy_Camponotus : EnemyBase
{
    ...
    public GameObject volcanoProjectorObj;
    ...
    const int effectRockImpactBig = 10;
    const int effectRockBreakBig = 11;
    const int throwVolcano = 0;
    const int RockAttackType = 5;
    const int RockBigAttackType = 7;
    ...
    protected override void SetLevelModifier()
    {
        ...
        if (healEffectIndex < effect.Length)
        {
            effect[healEffectIndex].prefab = healEffectPrefab[Mathf.Clamp(level, 0, healEffectPrefab.Length - 1)];
        }
    ...
    protected override void Update_StatusJudge()
    {
        ...
        if (volcanoProjectorObj)
        {
            bool volcanoEnabled = state == State.Attack && attackType == RockBigAttackType;
            if (volcanoProjectorObj.activeSelf != volcanoEnabled)
            {
                volcanoProjectorObj.SetActive(volcanoEnabled);
            }
        }
    }
    ...
    void EmitEffectImpact()
    {
        ...
        {
            if (attackType == RockBigAttackType)
            {
                EmitEffect(effectRockImpactBig);
                CameraManager.Instance.SetQuake(effect[effectRockImpactBig].pivot.position, 8, 4, 0, 0, 1.5f, 4f, dissipationDistance_Large);
            }
            else
            {
    ...
    void ThrowVolcano()
    {
        if (state == State.Attack)
        {
            throwing.ThrowStart(throwVolcano);
        }
    }
        ...
        if (targetTrans)
        {
            ...
            else
            {
                ...
                if (attackTemp == 0)
                {
                    ...
                    {
                        AttackBase(RockAttackType, 1.2f, 1.6f, 0, 160f / 60f, 160f / 60f + GetAttackInterval(1f, -3), 0, 1, false);
                    }
                }
                else
                {
                    noWeakAttackCount = 0;
                    if (level >= 5 && attackSaveWeak != RockBigAttackType && Random.Range(0, 2) == 0)
                    {
                        AttackBase(RockBigAttackType, 1.4f, 2.4f, 0, 130f / 60f, 130f / 60f + GetAttackInterval(1.5f, -3), 0, 1, false);
                        attackSaveWeak = RockBigAttackType;
                    }
                    else
                    {
