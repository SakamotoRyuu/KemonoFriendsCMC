...
public class Enemy_Periplaneta : EnemyBase {
    ...
    int levelUpCount;
    bool childEmitEnabled;
    const int levelUpMax = 3;
    const int EffEmitChild = 3;
    ...
    public override void ReceiveNoticeForHitAttack(DamageDetection damageReceiver)
    {
        if (level >= 5 && childEmitEnabled)
        {
            Rigidbody childRigid = Instantiate(childPrefab, childPivot.position, childPivot.rotation).GetComponent<Rigidbody>();
            childRigid.velocity = childVelocity;
            EmitEffect(EffEmitChild);
            childEmitEnabled = false;
        }
    }
