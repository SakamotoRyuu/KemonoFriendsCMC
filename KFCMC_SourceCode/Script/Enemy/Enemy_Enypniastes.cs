...
public class Enemy_Enypniastes : EnemyBase
{
    ...
    {
        bool margayVoiceSave = !fixKnockAmount && colorType == damageColor_Critical && enemyCanvasLoaded && enemyBalloonElements.MargayVoice.activeSelf && attacker != null && attacker == CharacterManager.Instance.pCon;
    ...
    protected override void Update_StatusJudge()
    {
        ...
        if (trapIntervalTimeRemain > 0)
        {
            trapIntervalTimeRemain -= deltaTimeCache;
        }
    }

    void ThrowDemonCore()
    {
        if (level >= 5 && trapIntervalTimeRemain <= 0)
        {
            throwing.ThrowStart(throw_DemonCore);
            trapIntervalTimeRemain = 3;
        }
    }
