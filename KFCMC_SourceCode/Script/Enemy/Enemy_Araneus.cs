...
public class Enemy_Araneus : EnemyBase
{
    ...
    public GameObject eggPrefabFast;
    ...
    const int eff_egg = 5;
    const int AttackIndexJump = 4;
    const int ThrowVolcano = 5;
        ...
        if (level >= 4)
        {
            if (target)
            {
                ...
                if (eggInterval <= 0f && GetCanControl())
                {
                    ...
                    GameObject eggInstance = Instantiate(level >= 5 ? eggPrefabFast : eggPrefab, eggPivot.position, eggPivot.rotation);
    ...
    protected override void Attack()
    {
        ...
        switch (attackTemp)
        {
            ...
            case 2:
                ...
                break;
            case 3:
                AttackBase(AttackIndexJump, 1.2f, 1.7f, 0, 105f / 60f + (IsSuperLevel ? 0f : 0.3f), 105f / 60f + GetAttackInterval(1.5f, -2), 0f);
                agent.radius = 0.05f;
                break;
        }
    }
