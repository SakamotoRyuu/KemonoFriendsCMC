...
public class Enemy_Glaucus : EnemyBaseBoss {
    ...
    int throwReservedIndexSecond = 0;
    int attackTypeSub = 0;
    int moveIndex = -1;
    ...
    const int throwIndexGhost = 0;
    const int throwIndexGhostSecond = 10;
    ...
    protected override void Update_StatusJudge() {
        ...
        if (state == State.Attack && throwRushTimeRemain > 0f) {
            ...
            if (throwRushIntervalRemain <= 0f) {
                throwRushIntervalRemain += (isRaw ? 0.0611111f : 0.0916667f);
                if (rushCount < (isRaw ? 25 : 17)) {
        ...
        // ノックダウン開始から石露出までリカバリーレートを0にするためのフラグ
        if (forceRecoveryRateZero && state != State.Damage)
        {
            forceRecoveryRateZero = false;
        }
    }
    ...
    void ThrowSpecialReady() {
        ...
        if (isRaw && throwReservedIndexSecond >= 3 && throwReservedIndexSecond <= 4)
        {
            throwing.ThrowReady(throwReservedIndexSecond);
        }
    }
    ...
    void ThrowSpecial() {
        ...
        if (isRaw)
        {
            throwing.ThrowStart(throwReservedIndexSecond);
        }
    }
    ...
    protected override void KnockHeavyProcess() {
        if (state != State.Damage || !isDamageHeavy) {
            ...
            agent.radius = 0.1f;
            agent.height = 0.1f;
            forceRecoveryRateZero = true; // ノックダウン開始時にリカバリーレートを0に
        }
    }
    ...
    void KnockStop() {
        ...
        forceRecoveryRateZero = false; // コアタイムスタート時にリカバリーレートも増加開始
    }
    ...
    protected override void Attack() {
        ...
        if (targetTrans) {
            ...
            {
                BattleStart();
                attackTypeSub = attackSave = attackTemp = 0;
            }
            ...
            if (isRaw && attackTemp >= 0 && attackTemp <= 3)
            {
                throwReservedIndexSecond = (throwReservedIndex + Random.Range(1, 4) * 2 - 1) % 8 + 1;
            }
            else
            {
                throwReservedIndexSecond = 0;
            }
        }
    }
    ...
    void ThrowReadyGhost()
    {
        throwing.ThrowReady(throwIndexGhost);
        if (isRaw)
        {
            throwing.ThrowReady(throwIndexGhostSecond);
        }
    }

    void ThrowStartGhost()
    {
        throwing.ThrowStart(throwIndexGhost);
        if (isRaw)
        {
            throwing.ThrowStart(throwIndexGhostSecond);
        }
    }
