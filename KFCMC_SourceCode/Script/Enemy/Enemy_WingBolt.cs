...
public class Enemy_WingBolt : EnemyBase
{
    ...
    protected override void Attack()
    {
        ...
        if (level >= 3)
        {
            attackRand = Random.Range(0, Mathf.Clamp(level, 1, 4));
        ...
        switch (attackRand)
        {
            ...
            case 1:
                ...
                SuperarmorStart();
                AttackBase(level >= 5 ? 5 : 2, 1.15f, 1.2f, 0, 85f / 60f, 85f / 60f + GetAttackInterval(1.5f, -2), 0, 1, true, 30f);
                agent.radius = 0.05f;
                break;
            case 2:
                fbStepTime = 15f / 60f;
                fbStepEaseType = EasingType.SineInOut;
                AttackBase(level >= 5 ? 4 : 1, 1.15f, 1.2f, 0, 75f / 60f, 75f / 60f + GetAttackInterval(1.5f, -2), 0);
                agent.radius = 0.05f;
                break;
            case 3:
    ...
    protected override void AttackContinuous()
    {
        ...
        if ((attackType == 1 || attackType == 4) && moveContinuous != 0)
        {
            Continuous_Approach(Mathf.Max(level >= 5 ? 15.12f : level >= 4 ? 12.6f : 10.5f, GetMaxSpeed(false, false, true)), 0.5f, 0.01f, true, false, 0.35f);
        }
    }
