...
public class Enemy_Tenodera : EnemyBase {
    ...
    int attackSave = -1;
    const int slashThrowIndex = 0;
    const int slashStrongThrowIndex = 1;
    const int manyThrowIndex = 2;
    ...
    private void ThrowStartSlashWave()
    {
        throwReserved = true;
        LockonEnd();
        if (level >= upperAimLevel)
    ...
    private void LateUpdate()
    {
        if (upperAimEnabled && state == State.Attack)
        {
            ...
            {
                upperAimAngleSave = Mathf.Clamp(GetTargetUpperAngle(upperAimAnglePivot), 0, 45) * -1;
            }
        ...
        if (state == State.Attack && throwReserved)
        {
            throwing.ThrowStart(level >= upperAimLevel && scytheEnabled ? slashStrongThrowIndex : slashThrowIndex);
            throwReserved = false;
