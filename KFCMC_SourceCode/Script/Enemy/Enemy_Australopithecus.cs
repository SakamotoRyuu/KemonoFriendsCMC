...
public class Enemy_Australopithecus : EnemyBase
{
    ...
    const float levelFiveShoutMul = 3f;
    ...
    protected override void Awake()
    {
        ...
        {
            for (int i = coreEffectIndex; i < coreHealIndex && i < effect.Length; i++)
            {
        ...
        {
            for (int i = coreHealIndex; i < coreHealIndex && i < effect.Length; i++)
            {
    ...
    protected override void SetLevelModifier()
    {
        ...
        int levelTemp = Mathf.Clamp(level, 0, 5);
        for (int i = 0; i < weaponSet.Length; i++)
        {
            if (weaponSet[i].obj != null)
            {
                weaponSet[i].obj.SetActive(weaponSet[i].level == levelTemp && weaponSet[i].isLeft == isLefty);
            }
        }
        if (level == 5)
        {
    ...
    protected override void Update_StatusJudge()
    {
        ...
        if (shoutInterval > 0f)
        {
            shoutInterval -= deltaTimeCache * (level == 5 ? levelFiveShoutMul : 1f);
        }
        if (enemyBalloonLoaded && 
            target && 
            ((isCoreShow && nowHP < GetMaxHP()) || level == 5) && 
            !enemyBalloonElements.PaperPlane.activeSelf && 
            !enemyBalloonElements.MargayVoice.activeSelf)
        {
            shoutReadyTime += deltaTimeCache * (level == 5 ? levelFiveShoutMul : 1f);
        }
        else
        {
        ...
        if (!isHomoChild &&
            targetTrans && 
            shoutInterval <= 0f &&
            shoutReadyTime >= 10f * CharacterManager.Instance.riskyDecrease &&
            GetTargetDistance(true, true, false) <= 6f * 6f &&
            StageManager.Instance.dungeonController &&
            StageManager.Instance.dungeonController.australopithecusShoutInterval <= 0f &&
            StageManager.Instance.dungeonController.EnemyCountDistance(trans.position, 50f, true) >= 2 &&
            !enemyBalloonElements.PaperPlane.activeSelf &&
            !enemyBalloonElements.MargayVoice.activeSelf)
        {
            shoutReadied = true;
    ...
    public override bool SetConcentratedAttack(Vector3 parentPosition, GameObject decoy, CharacterBase attacker = null)
    {
        if (base.SetConcentratedAttack(parentPosition, decoy, attacker))
        {
            ...
            if (isCoreShow && coreHideTimeRemain <= 0f && nowHP >= GetMaxHP())
            {
                ...
                EmitEffect(coreHealIndex + level);
            }
            return true;
        }
        return false;
    ...
    protected override void Attack()
    {
        ...
        if (shoutReadied || level == 5)
        {
            AttackBase(4, 0f, 0f, 0, 66f / 30f, 66f / 30f + (level == 5 ? Random.Range(3.6f, 4.0f) : 0f), 0f, 1f, false);
            SuperarmorStart();
            shoutInterval = 10f;
            StageManager.Instance.dungeonController.australopithecusShoutInterval = 4f * (level == 5 ? 1f / levelFiveShoutMul : 1f);
        }
        else
        {
            if (fatigue >= 8f)
