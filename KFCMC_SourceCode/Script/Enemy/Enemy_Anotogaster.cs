...
public class Enemy_Anotogaster : EnemyBase {
    ...
    const int EffectSpecialDodge = 6;
    const int AttackDetectionPropellerNormal = 5;
    const int AttackDetectionPropellerBlownAway = 6;
    ...
    protected override void AttackContinuous() {
        ...
        if (moveIndex >= 0 && moveIndex < movePivot.Length && movePivot[moveIndex]) {
            ApproachTransformPivot(movePivot[moveIndex], Mathf.Clamp(GetMaxSpeed() * 1.5f, 9f, 24f), 0.5f, 0.025f, true);
        }
    }

    void AttackStartPropeller()
    {
        if (level >= 5)
        {
            AttackStart(AttackDetectionPropellerBlownAway);
        }
        else
        {
            AttackStart(AttackDetectionPropellerNormal);
        }
    }

    void AttackEndPropeller()
    {
        AttackEnd(AttackDetectionPropellerNormal);
        AttackEnd(AttackDetectionPropellerBlownAway);
    }
