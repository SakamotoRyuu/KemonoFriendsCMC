...
public class Enemy_Debris : EnemyBase
{
    ...
    //public GameObject normalDamageDetectionObj;
    //public GameObject guardDamageDetectionObj;
    public GameObject additionalNeedleParent;
    ...
    bool needleAttackEnabled;
    bool bigBlockEnabled;
    float targetExistTimeForChild;
    ...
    static readonly int[] rushAttackDetectionIndexes = new int[] { 0, 2 };
    const int throwIndexBigBlock = 10;
    ...
    protected override void SetLevelModifier()
    {
        dropRate[0] = 250;
        dropRate[1] = 250;
        //bool guardEnabled = level >= 5;
        //normalDamageDetectionObj.SetActive(!guardEnabled);
        //guardDamageDetectionObj.SetActive(guardEnabled);
    }
    ...
    protected override void Update_StatusJudge()
    {
        ...
        if (GetCanControl() && additionalNeedleParent.activeSelf)
        {
            additionalNeedleParent.SetActive(false);
        }
        if (target)
        {
            targetExistTimeForChild += deltaTimeCache;
        }
        else
        {
            targetExistTimeForChild = 0;
        }
    }
    ...
    void ThrowReadySpecial()
    {
        if (state == State.Attack)
        {
            ...
            if (bigBlockEnabled)
            {
                throwing.ThrowReady(throwIndexBigBlock);
            }
        }
    }
    ...
    void ThrowStartSpecial()
    {
        if (state == State.Attack)
        {
            if (throwNow < throwMax)
            {
                ...
                throwNow++;
                if (bigBlockEnabled && throwNow == throwMax)
                {
                    throwing.ThrowStart(throwIndexBigBlock);
                }
            }
        }
    }
    ...
    protected override void Attack()
    {
        ...
        needleAttackEnabled = level >= 5;
        bigBlockEnabled = false;
        SuperarmorStart();
        ...
        if (attackMainTemp == 0)
        {
            if (level <= 1 || attackSubTemp == 0)
            {
                ...
                if (needleAttackEnabled)
                {
                    additionalNeedleParent.SetActive(true);
                }
            }
            else
            {
        ...
        else
        {
            if (level <= 3 || attackSubTemp == 0)
            {
                ...
                bigBlockEnabled = level >= 5 && targetExistTimeForChild >= 12f;
            }
            else
            {
    ...
    public void AttackStartRush()
    {
        int length = needleAttackEnabled ? rushAttackDetectionIndexes.Length : 1;
        for (int i = 0; i < length; i++)
        {
            AttackStart(rushAttackDetectionIndexes[i]);
        }
    }

    public void AttackEndRush()
    {
        int length = needleAttackEnabled ? rushAttackDetectionIndexes.Length : 1;
        for (int i = 0; i < length; i++)
        {
            AttackEnd(rushAttackDetectionIndexes[i]);
        }
    }

    public void EmitEffectNeedle(int index)
    {
        if (needleAttackEnabled)
        {
            EmitEffect(index);
