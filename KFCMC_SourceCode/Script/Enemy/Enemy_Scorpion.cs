...
public class Enemy_Scorpion : EnemyBase
{
    ...
    public float hammerSeparateDistance;
    public Transform adjustScalePivot;
    ...
    const int ThrowNormalMoltIndex = 0;
    const int ThrowBurstMoltIndex = 1;
    ...
    {
        if (roarEnabled && adjustScalePivot)
        {
            GameObject mapChipObj = base.SetMapChip();
            if (mapChipObj)
            {
                mapChipObj.transform.SetParent(adjustScalePivot);
                mapChipObj.transform.localScale = Vector3.one;
            }
            return mapChipObj;
        }
        else
        {
            return base.SetMapChip();
        }
    }
    ...
    void ThrowStartMolt()
    {
        ThrowStartChangeVelocity(moltIndexSave);
    }
        ...
        else
        {
            ...
            switch (attackTemp)
            {
                ...
                case 3:
                    ...
                    throwing.ThrowReady(moltIndexSave);
                    break;
                case 4:
                    if (roarEnabled)
                    {
                    ...
                    fbStepMaxDist = 2f;
                    fbStepTime = 0.5f;
                    SeparateFromTarget(hammerSeparateDistance);
                    break;
            }
        }
    }
    ...
    protected override void AttackContinuous()
    {
        ...
        if (moveIndex >= 0 && moveIndex < movePivot.Length && movePivot[moveIndex] && targetTrans)
        {
            float sqrDistPivot = MyMath.SqrMagnitudeIgnoreY(trans.position, movePivot[moveIndex].position);
            float sqrDistTarget = MyMath.SqrMagnitudeIgnoreY(trans.position, targetTrans.position);
            if (sqrDistTarget > sqrDistPivot)
            {
