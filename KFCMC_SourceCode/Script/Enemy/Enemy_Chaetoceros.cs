...
public class Enemy_Chaetoceros : EnemyBase
{
    ...
    bool isNeedleWallBreak;
    float searchAreaEnablizeTimer;

    const int ThrowBigNeedleDefault = 5;
    const int ThrowBigNeedleWallBreak = 6;
    ...
    protected override void Attack()
    {
        ...
        if (targetSearchAreaIndex == 1)
        {
            attackTemp = 1;
        }
        attackSave = attackTemp;
        isNeedleWallBreak = level >= 5;
        if (attackTemp == 0)
        {
    ...
    void ThrowReadyBigNeedle()
    {
        if (isNeedleWallBreak)
        {
            throwing.ThrowReady(ThrowBigNeedleWallBreak);
        }
        else
        {
            throwing.ThrowReady(ThrowBigNeedleDefault);
        }
    }

    void ThrowStartBigNeedle()
    {
        if (isNeedleWallBreak)
        {
            throwing.ThrowStart(ThrowBigNeedleWallBreak);
        }
        else
        {
            throwing.ThrowStart(ThrowBigNeedleDefault);
        }
    }
