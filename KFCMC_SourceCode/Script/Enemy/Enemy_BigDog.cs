...
public class Enemy_BigDog : EnemyBaseBoss {
    ...
    public Vector3 finalFormCanvasPosition;
    ...
    */
    ...
    public override void SetSandstarRaw() {
        ...
        if (eventState == EventState.Escaped || eventState == EventState.Restarted) {
            // defensePower = 50;
            defensePower = 0;
        }
        lastDamagedTime = 0f;
    ...
    protected override void KnockLightProcess() {
        if (!isSuperarmor){
            if (!isReaper && eventState == EventState.Invinsible) {
    ...
    void EventStart() {
        ...
        if (enemyCanvasLoaded) {
            enemyCanvasElements.SetActiveAll(false);
        }
    ...
    void StartFinalForm() {
        ...
        if (canvasPivot)
        {
            canvasPivot.localPosition = finalFormCanvasPosition;
        }
    }
    ...
    protected override void Update_StatusJudge() {
        ...
        if (restartEventTimeRemain > 0f) {
            ...
            if (restartEventTimeRemain <= 0f) {
                eventState = EventState.Restarted;
        ...
        if (eventState == EventState.Swallow) {
    ...
    int GetAttackType() {
        ...
        if (eventState == EventState.Wait && nearEnabled && targetTrans == CharacterManager.Instance.playerSearchTarget){
            attackTemp = 3; //EVENT
            eventReadied = true;
        }
    ...
    protected override void Attack() {
        ...
        if (attackTemp == 0) {
            ...
            if (eventState == EventState.Wait && targetTrans == CharacterManager.Instance.playerSearchTarget) {
    ...
    public override void AttackStart(int index) {
        if (attackType != 10 || eventState != EventState.Swallow) {
    ...
    protected override void DeadProcess() {
        ...
        if (finalFormFlag) {
            CharacterManager.Instance.ReserveAllFriendsAgentReset();
        }
        base.DeadProcess();
    }
    ...
    public override float GetKnockGaugeNow()
    {
        ...
        else
        {
            if (eventState == EventState.Escaped || eventState == EventState.Restarted)
            {
                ...
                for (int i = 0; i < scarDamage.Length; i++)
                {
                    sum += Mathf.Clamp(scarDamage[i], 0, scarCondition);
                }
                return scarCondition * scarDamage.Length - sum;
            }
            else if (eventState == EventState.Wait || eventState == EventState.Swallow)
            {
                return 0;
            }
            else
    ...
    public override float GetKnockGaugeMax()
    {
        ...
        else
        {
            if (eventState == EventState.Escaped || eventState == EventState.Restarted)
            {
                return scarCondition * scarDamage.Length;
            }
            else if (eventState == EventState.Wait || eventState == EventState.Swallow)
            {
                return 1;
    ...
    {
        return finalFormFlag && state == State.Damage;
    }
    ...
    public override float GetKnockRecoveryGaugeRate()
    {
        if (state == State.Damage)
        {
            ...
            float stiffTime = GetLightStiffTime();
            if (stiffTime > 0)
            {
