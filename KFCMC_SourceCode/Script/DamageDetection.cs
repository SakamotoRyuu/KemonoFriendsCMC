...
public class DamageDetection : MonoBehaviour {
    ...
    public virtual void PlayHitEffect(Vector3 effectPosition, int colorType, bool isColorOverrided, AttackDetection attackDetection)
    {
        ...
        if (hitEffectNum < poolParticleNames.Length && hitEffectNum < poolAudioNames.Length && ParticleSystemPool.Instance && AudioSourcePool.Instance)
        {
            ...
            {
                ParticleSystemPool.Instance.Play(poolParticleNames[hitEffectNum], effectPosition, timeDifference);
                float volumeRate = CharacterManager.Instance.GetDamageSoundVolume(colorType);
                if (isColorOverrided)
                {
                    volumeRate *= 0.4f;
                }
