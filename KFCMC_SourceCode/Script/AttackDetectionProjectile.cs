...
public class AttackDetectionProjectile : AttackDetectionSick {
    ...
    public GameObject instantCollisionCollider;
    ...
    protected override bool HitAttack(ref DamageDetection targetDD)
    {
        bool answer = base.HitAttack(ref targetDD);
        if (answer && instantCollisionCollider)
        {
            StartCoroutine(InstantCollision());
        }
        return answer;
    }

    IEnumerator InstantCollision()
    {
        if (instantCollisionCollider)
        {
            instantCollisionCollider.SetActive(true);
        }
        yield return new WaitForFixedUpdate();
        if (instantCollisionCollider)
        {
            instantCollisionCollider.SetActive(false);
        }
    }
