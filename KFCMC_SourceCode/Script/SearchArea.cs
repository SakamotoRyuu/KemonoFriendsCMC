...
public class SearchArea : MonoBehaviour {
                ...
                if (targetCBase && targetCBase.searchTarget.Length > 1) {
                    ...
                    for (int i = 0; i < targetCBase.searchTarget.Length; i++) {
                        if (targetCBase.searchTarget[i] != null && targetCBase.searchTarget[i].activeInHierarchy) {
                            float distTemp = (targetCBase.searchTarget[i].transform.position - transPos).sqrMagnitude;
            ...
            if (count > 0) {
                for (int i = count - 1; i >= 0; i--) {
                    if (!target[i].obj || !target[i].obj.activeInHierarchy) {
                        ...
                        float sqrDist = (transPos - targetPos).sqrMagnitude;
                        if ((target[i].rayReaching && sqrDist <= dontForgetDistance * dontForgetDistance) || (!target[i].rayReaching && sqrDist <= seeThroughDistance * seeThroughDistance)) { 
                            ...
                            if (linecastIntervalRemain < 0f) {
                                target[i].rayReaching = !Physics.Linecast(transPos, targetPos, fieldLayerMask, QueryTriggerInteraction.Ignore);
                                if (multiCheck && !target[i].rayReaching) {
                                    target[i].rayReaching = !Physics.Linecast(transPos + multiCheckOffset, targetPos, fieldLayerMask, QueryTriggerInteraction.Ignore);
                                }
            ...
            if (count > 0) {
                ...
                for (int i = 0; i < count; i++) {
                    if (target[i].forgetTimeRemain >= 0) {
                        ...
                        distNow = (transPos - target[i].trans.position).sqrMagnitude;
                ...
                if (index >= 0 && nowTarget != target[index].obj) {
                    if (isPlayer && nowTarget && nowTarget.activeInHierarchy && targetUpdatingMargin > 0f) {
                        if (targetChangeInterval <= 0f) {
                            float distOld = (transPos - nowTargetTrans.position).sqrMagnitude;
                            if (priorityEffectRate > 0f) {
    ...
    void OnTriggerEnter(Collider other) {    
        if (other.CompareTag(searchTag)) {
            ...
            // PaperPlane
            if (parentCBase && parentCBase.isEnemy) {
                Decoy decoy = other.gameObject.GetComponent<Decoy>();
                if (decoy)
                {
                    bool isAttracterPlayer = decoy.NoticeEnemyAttracted();
                    parentCBase.Attraction(other.gameObject, CharacterBase.AttractionType.PaperPlane, true, 0.6f, isAttracterPlayer);
                }
            }
        }
    }
        ...
        if (count > 0) {
            for (int i = 0; i < count; i++) {
                if (target[i].obj) {
                    int weighting = 1;
                    Vector3 targetPos = target[i].trans.position;
                    float sqrDist = (targetPos - transPos).sqrMagnitude;
                    if (sqrDist < 36f) {
                        weighting++;
                        if (sqrDist < 16f) {
                            weighting++;
                        ...
                        if (target[i].obj && target[i].obj != nowTarget) {
                            ...
                            if (otherTargetCBase != nowTargetCBase) {
                                ...
                                if (otherTargetCapsule) {
                                    ...
                                    if (MyMath.SqrMagnitudeIgnoreY(transPos, target[i].trans.position) - radius * radius <= condSqrDist) {
                                        return true;
                                    }
                                }
                            }
        ...
        if (lockTarget && lockTarget.gameObject.activeInHierarchy) {
            ...
            for (int i = 0; i < lockTarget.searchTarget.Length; i++) {
                if (lockTarget.searchTarget[i] != null && lockTarget.searchTarget[i].activeInHierarchy) {
                    float distTemp = (lockTarget.searchTarget[i].transform.position - transPos).sqrMagnitude;
                    if (priorityEffectRate > 0f) {
        ...
        get {
            ...
            if (count > 0) {
                for (int i = 0; i < count; i++) {
                    if (target[i].trans) {
                        Vector3 targetPos = target[i].trans.position;
                        if ((transPos - targetPos).sqrMagnitude <= seeThroughDistance * seeThroughDistance) {
                            answer = true;
                            break;
                        } else if (!Physics.Linecast(transPos, targetPos, fieldLayerMask, QueryTriggerInteraction.Ignore)) {
                            answer = true;
                            break;
                        }
                    }
    ...
    public bool GetPlayerFindable {
        get {
            ...
            if (CharacterManager.Instance && CharacterManager.Instance.playerSearchTarget) {
                ...
                if (sqrDist <= 10f * 10f) {
                    answer = true;
                } else if (sqrDist <= 30f * 30f && !Physics.Linecast(transPos, targetPos, playerFindableLayerMask, QueryTriggerInteraction.Ignore)) {
                    answer = true;
                }
            }
            return answer;
    ...
    */
    public void ChangeTarget(float time = 5.0f) {
        if (nowTarget && GetTargetNum > 1) {
            GameObject nextTarget = nowTarget;
            float baseDist = (nowTarget.transform.position - transPos).sqrMagnitude;
            ...
            for (int i = 0; i < count; i++) {
                if (target[i].obj && nowTarget != target[i].obj) {
                    ...
                    if (target[i].rayReaching) {
                        float distTemp = (targetPos - transPos).sqrMagnitude;
