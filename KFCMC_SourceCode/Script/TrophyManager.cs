...
public class TrophyManager : SingletonMonoBehaviour<TrophyManager> {
    ...
    private bool steamAchievementChecked;
    ...
    public bool IsTrophyHad(int index) {
        if (!GameManager.Instance.IsPlayerAnother) {
            int arrayNum = index / GameManager.bitUnit;
            int bitNum = 1 << (index % GameManager.bitUnit);
    ...
    private void CheckTrophy_Complete() {
        if (GameManager.Instance.save.config[GameManager.Save.configID_DisableTrophy] == 0) {
            ...
            if (answer) {
                GetTrophy(t_CompleteTrophy, t_CompleteTrophy / GameManager.bitUnit, 1 << (t_CompleteTrophy % GameManager.bitUnit));
    ...
    public void CheckSteamAchievement() {
        if (!steamAchievementChecked && GameManager.Instance && GameManager.Instance.save.config[GameManager.Save.configID_DisableTrophy] == 0) {
            int arrayNum = t_CompleteTrophy / GameManager.bitUnit;
            int bitNum = 1 << (t_CompleteTrophy % GameManager.bitUnit);
            if ((GameManager.Instance.save.trophy[arrayNum] & bitNum) != 0) {
                GameManager.Instance.SetSteamAchievement(GameManager.SteamAchievementName.ALLTROPHIES);
                steamAchievementChecked = true;
            }
        }
    }
                    ...
                    switch (index) {
                        ...
                        case t_DefeatAllType:
                            if (GameManager.enemyMax * GameManager.enemyLevelMax <= GameManager.Instance.save.defeatEnemy.Length) {
                                answer = true;
                                // 試練の洞窟限定セルリアンを除く
                                for (int i = 0; i < GameManager.enemyNormalMax; i++) {
                        ...
                        case t_CompleteEnemyBook:
                            if (GameManager.enemyMax * GameManager.enemyLevelMax <= GameManager.Instance.save.defeatEnemy.Length) {
                                answer = true;
                                // 試練の洞窟限定セルリアンを除く
                                for (int i = 0; i < GameManager.enemyNormalMax && answer; i++) {
                                    // 試練の洞窟限定レベルを除く
                                    for (int j = 0; j < GameManager.enemyNormalLevelMax; j++) {
